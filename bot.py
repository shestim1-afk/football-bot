...
def cleanup_state(live_fixture_ids: set[int]):
    ...
    # v10.44p: Clean event fast lane if fixture finished
    global _event_fast_lane_fid
    if _event_fast_lane_fid is not None and _event_fast_lane_fid not in live_fixture_ids:
        log.info(f"  v10.44p: Event fast lane cleared (F{_event_fast_lane_fid} finished)")
...
def get_sot_based_interval(fid: int, base_interval: int) -> int:
    ...
    if both_teams_signaled:
        ...
        for (sfid, stid), sinfo in signaled_teams.items():
            if sfid == fid:
                st_gps = sinfo.get("gps", 0)
                st_sot = sinfo.get("sot", 0)
                st_accel = sinfo.get("accel_count", 0)
                st_minute = sinfo.get("minute", 0)
                if st_gps >= 75 and st_sot >= 3 and st_accel >= 1 and st_minute < 75:
                    _skip_slowdown = True
                    log.info(f"  v10.44p: F{fid} both signaled but {sinfo.get('team_name','?')} GPS={st_gps} SOT={st_sot} accel={st_accel} — skipping 3x slowdown")
                    break
...
_event_fast_lane_fid: int | None = None   # fixture ID currently in fast lane
...
# Value: dict mapping team_id -> [(player_name, sot_count), ...]
_player_sot_cache: dict[int, dict[int, list[tuple[str, int]]]] = {}
...
def fetch_top_sot_players(
    ...
    if team_id in _cached:
        return _cached[team_id][:max_players]
    ...
    try:
        ...
        for ev in events:
            etype = ev.get("type", "")
            detail = ev.get("detail", "")
            if (etype == "Shot" and detail == "On target") or (etype == "Goal" and detail != "Own Goal"):
        ...
        result = _fixture_data.get(team_id, [])[:max_players]
        return result
    except Exception as e:
...
def update_event_fast_lane() -> None:
    """v10.44p: Identify the single hottest fixture for 10s event polling.

    Criteria: GPS>=65 AND SOT>=2 AND (accelerating OR in pressure_accelerating).
    Only ONE fixture at a time to cap credit cost.
    Resets _event_fast_lane_fid when no fixture qualifies or fixture finishes.
    """
    global _event_fast_lane_fid
    _best_fid = None
    _best_score = -1
    for (fid, tid), sinfo in team_state.items():
        ...
        _accel = sinfo.get("accel_count", 0)
        _minute = sinfo.get("last_minute", 0)
        # Must meet minimum thresholds and be in signal window
        if _gps >= EVENT_FAST_LANE_MIN_GPS and _sot >= EVENT_FAST_LANE_MIN_SOT and _minute >= 21:
            # Score: GPS + accel bonus (prefer accelerating fixtures)
            _score = _gps + (_accel * 5)
            # Bonus for pressure acceleration
    ...
    if _best_fid != _event_fast_lane_fid:
        ...
        elif _event_fast_lane_fid is not None:
            log.info(f"  v10.44p: Event fast lane cleared (no qualifying fixture)")
        _event_fast_lane_fid = _best_fid


def poll_event_fast_lane(client: httpx.Client) -> None:
    """v10.44p: Poll events for the hottest fixture every 10s.

    Detects SOT jumps that statistics hasn't caught up to yet.
    When events SOT > stats SOT, it means a new shot happened but
    the stats feed hasn't updated — the next stats poll will see it
    with up to 10s less lag.
    Daily credit cap: 1500 (safety limit, ~2.5hr of continuous 10s polling).
    """
    global _event_fast_lane_last, _event_fast_lane_credits_today
    now = time.time()
    if _event_fast_lane_fid is None:
        return
    ...
    try:
        data = api_get(client, "/fixtures/events", {"fixture": _event_fast_lane_fid})
        events = data.get("response", [])
        _event_fast_lane_last = now
        _event_fast_lane_credits_today += 1

        # Count event-based SOT per team
        ev_sot: dict[int, int] = {}
        latest_min = 0
        for ev in events:
            etype = ev.get("type", "")
        ...
        if latest_min > 0:
            _latest_sot_event_minute[_event_fast_lane_fid] = latest_min

        # Compare with stats-based SOT to detect lag
        stats_best = get_fixture_best_sot(_event_fast_lane_fid)
        ev_best = max(ev_sot.values()) if ev_sot else 0
        if ev_best > stats_best:
            lag_sot = ev_best - stats_best
            log.info(
                f"  v10.44p FAST LANE: F{_event_fast_lane_fid} events SOT={ev_best} > "
                f"stats SOT={stats_best} (+{lag_sot} shot(s) ahead)"
            )
        else:
            log.debug(f"  v10.44p FAST LANE: F{_event_fast_lane_fid} events SOT={ev_best} stats SOT={stats_best} (in sync)")
    except Exception as e:
        log.warning(f"  v10.44p FAST LANE error for F{_event_fast_lane_fid}: {e}")
    ...
    try:
        ...
        for update in results:
            ...
            if text == "/help":
                ...
                # v10.44p: Show event fast lane status
                _fl_msg = "idle"
                if _event_fast_lane_fid is not None:
                    _fl_msg = f"F{_event_fast_lane_fid} ({_event_fast_lane_credits_today} credits today)"
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    for tid, tname in ((home_tid, home["name"]), (away_tid, away["name"])):
        ...
        if minute >= FRESHNESS_HARD_STOP:
            ...
            is_event_burst = (events_sot > stats_sot and events_sot >= 3)
            ...
            if _has_recency_data:
                ...
                if not _has_fresh_pressure:
                    ...
                    log.info(
                        ...
                        f"no fresh pressure (SOT_5m=+{_sot_d5} GPS_chg={_gps_chg} accel={accel_count} GPS={gps:.0f} SOT={sot})"
                    )
                    continue
        ...
        if is_first_signal_only_mode() and sig_count >= 1:
            _fso_sot_at_sig = team_sig.get("sot_at_last_signal", 0) if team_sig else 0
            _fso_xg_at_sig = team_sig.get("xg_at_last_signal", None) if team_sig else None
            _fso_sot_rise = sot - _fso_sot_at_sig
            ...
            _fso_buildup = _fso_sot_rise >= 3 or _fso_xg_rise >= 0.50
            is_exceptional = (
            ...
            if not is_exceptional:
                ...
                continue
            _override_reason = (
                'SOT-BURST' if fid in sot_burst_fixtures
        ...
        # v10.44n: Top SOT players from /fixtures/events (player-level data)
        # v10.21 note: /fixtures/statistics has no player array, but events does.
        top_sot_str = ""
        top_sot_players = fetch_top_sot_players(client, fid, tid, max_players=3)
        if top_sot_players:
            _player_parts = [f"{name} ({cnt})" for name, cnt in top_sot_players]
            top_sot_str = ", ".join(_player_parts)
        ...
        msg = (
            ...
            f"xG: {xg_str} | Big Chances: {big_chances} | Off-target: {shots_off_target} | Corners: {corners}\n"
        ...
        signal_outcomes.append({
            ...
            # v10.44n: Top SOT players for signal context
            "top_sot_players": [{"name": n, "sot": c} for n, c in top_sot_players] if top_sot_players else [],
...
def main():
    ...
    log.info("     Quality chances (GK save/attempt) — more predictive than corners")
    log.info("     When BC available: corners cap 4→2, BC gets up to 3 pts")
    log.info("     When BC missing (European): corners 4 pts, BC 0 pts (unchanged)")
    log.info("  2) N/A SAFETY: safe_int now explicitly handles N/A/None/null strings")
    log.info("     Previously relied on ValueError exception — now explicit check")
    log.info("  3) SIG-REVIVE: FIRST_SIGNAL_ONLY re-checks if score changed since signal")
    log.info("  4) STALE ACCEL FIX: +3 dampener uses 5m SOT delta (not stale accel_count)")
    log.info("  5) TOTAL_SHOTS INFERENCE: SOT>0 but total_shots=0 inferred as SOT+off-target")
    log.info("  6) SHOTS INSIDE BOX already in GPS as IB ratio (up to 22-27 pts)")
    log.info("")
    log.info("v10.42 CHANGE (adaptive GPS):")               
    log.info("  xG component kept for domestic leagues (PL, Serie A, La Liga, etc.)")               
    log.info("  European comps (CL/EL): xG never returned, 18pts redistributed")               
    log.info("  Domestic: SOT 28, IB 22, Accel 22 + xG 18 (original)")               
    log.info("  European: SOT 35, IB 27, Accel 28, no xG (redistributed)")               
    log.info("")               
    log.info("v10.41 CHANGE (single controlled test):")               
    log.info("  Removed SOT>=5 bypass from freshness gates at 61'+")               
    log.info("  Stale pressure now blocks ALL signals regardless of SOT count")               
    log.info("  GPS floor (80+) and hard stop (80') from v10.40: UNCHANGED")               
    log.info("")               
    log.info("v10.40 CHANGES (tighter late-game gates):")
    log.info("  1. FRESHNESS_HARD_STOP: 86' -> 80' (block 80'+ unless event-confirmed SOT burst)")
    log.info("  2. Stale CRITICAL at 61'+: TAG -> BLOCK (unless SOT>=5 extreme danger)")
    log.info("  3. Late GPS floor: CRITICAL at 61-79' needs GPS>=80 (unless SOT>=5)")
    log.info("  4. Expected: fewer but higher-quality late signals")
    log.info("")
    ...
    log.info("  Rationale: 3+ SOT after scoring = sustained dominance, not stat inflation.")
    log.info("  EW still fully suppressed (no SOT proof of danger).")
    log.info("")
    log.info("v10.35 CHANGES (context-aware CRITICAL):")
    log.info("  CRITICAL (SOT>=3) at 61-85' without fresh pressure: TAGGED, not blocked.")
    log.info("  SOT>=3 proves danger exists — stale is weaker but still valid signal.")
    log.info("  EW without freshness at 61-85': still blocked (no SOT proof).")
    log.info("  86'+ hard stop unchanged (event-burst SOT only).")
    log.info("  is_stale_critical field added to outcomes for WR tracking.")
    log.info("")
    log.info("v10.34 CHANGES (fresh pressure gates + stricter losing filter):")
    log.info("  61-85': freshness gate (SOT rising, GPS rising, accel, or GPS>=85+IB>=60%)")
    log.info("  76'+: additionally requires IB>=60% (stricter quality gate)")
    log.info("  86'+: hard stop unless event-confirmed SOT burst (events_sot > stats_sot >=3)")
    log.info("  Losing teams CRITICAL: GPS>=80+IB>=65%+SOT>=4 (was no gate for CRITICAL)")
    log.info("")
