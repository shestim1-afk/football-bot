...
# Friendly league IDs are auto-detected from API response (no hardcoding).
FRIENDLY_THRESHOLD = 5  # include friendlies if fewer main-league matches than this
active_friendly_ids: set[int] = set()  # auto-discovered friendly league IDs for today
...
def do_discovery(client: httpx.Client) -> bool:
    ...
    tracked = [f for f in cached_fixtures if is_tracked_league(f["league"]["id"])]
...
def is_tracked_league(lid: int) -> bool:
    """Check if a league ID is tracked (main 20 leagues OR active friendlies)."""
    return lid in LEAGUE_IDS or lid in active_friendly_ids
    ...
    try:
        ...
        for f in all_fixtures:
            ...
            if lid in LEAGUE_IDS:
                ...
                friendly_hours.append((lid, hour, f["league"].get("name", "?")))
        
        schedule_date = today_str
        schedule_last_fetch = time.time()
        
        # If few main-league matches, also include friendlies
        global active_friendly_ids
        if len(kickoff_hours) < FRIENDLY_THRESHOLD and friendly_hours:
            added = 0
            for fid, hour, name in friendly_hours:
                active_friendly_ids.add(fid)
                kickoff_hours.append(hour)
                added += 1
            log.info(
                f"  Friendly fallback activated: {added} friendly match(es) added "
                f"(only {len(kickoff_hours) - added} main-league matches)"
            )
        else:
            if active_friendly_ids:
                log.info(f"  Clearing {len(active_friendly_ids)} friendly league IDs (enough main matches)")
            active_friendly_ids = set()
...
def main():
    ...
    log.info(f"  Friendly fallback: ON when <{FRIENDLY_THRESHOLD} main-league matches (auto-detect IDs)")
    ...
    with httpx.Client(timeout=30.0) as client:
        while True:
            ...
            has_tracked_live = bool(
                [f for f in cached_fixtures if is_tracked_league(f["league"]["id"])]
                ...
                try:
                    ...
                    has_tracked_live = bool(
                        [f for f in cached_fixtures
                         if is_tracked_league(f["league"]["id"])]
                    )
                    discovery_interval = get_discovery_interval(
                        budget, has_tracked_live, has_candidates
                    )
