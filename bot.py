...
# --- v10.31: Events-based SOT verification (API lag bypass) ---
# API-Football's /fixtures/statistics endpoint lags 2-3 min behind
# real-time. The /fixtures/events endpoint updates instantly.
# When we detect stats lag (InBox jumping but SOT frozen), we verify
# SOT via events. Budget: max 5 extra credits/day.
EVENTS_VERIFY_DAILY_BUDGET = 5
events_verify_calls_today: int = 0
events_verify_date: str = ""  # track date for daily reset
# Cache: (fixture_id, team_id) -> {"sot": int, "ts": float}
# Prevents re-verifying the same team within 3 minutes
events_verified_sot: dict[tuple[int, int], dict] = {}
EVENTS_VERIFY_COOLDOWN = 180  # 3 minutes between re-verifications per team
...
def _reset_events_verify_daily() -> None:
    """Reset events verification budget at date change."""
    global events_verify_calls_today, events_verify_date
    today_str = datetime.now(BULGARIA_TZ).strftime("%Y-%m-%d")
    if events_verify_date != today_str:
        events_verify_calls_today = 0
        events_verify_date = today_str
        events_verified_sot.clear()


def detect_stats_lag(
    sot: int, total_shots: int, shots_inside_box: int,
    state: dict | None, fid: int, tid: int,
) -> bool:
    """v10.31: Detect if API-Football stats endpoint is lagging.

    Lag signature: shots/InBox increased significantly but SOT frozen.
    SOT is a subset of InBox shots - if InBox jumped 2+ but SOT didn't
    move, the aggregated stats are stale.
    Only worth verifying when SOT >= 2 (signal-relevant range).
    """
    if not state:
        return False
    if sot < 2:
    ...
    # Don't re-verify same team within cooldown
    cached = events_verified_sot.get((fid, tid))
    if cached and (time.time() - cached["ts"]) < EVENTS_VERIFY_COOLDOWN:
        return False

    prev_ib = state.get("last_shots_inside_box", 0)
    prev_sot = state.get("last_sot", 0)
    prev_total = state.get("last_total_shots", 0)

    ib_delta = shots_inside_box - prev_ib
    sot_delta = sot - prev_sot
    total_delta = total_shots - prev_total

    # Primary: InBox jumped 2+ but SOT flat or decreased
    if ib_delta >= 2 and sot_delta <= 0:
        return True

    # Secondary: total shots jumped 3+ and InBox 1+ but SOT flat
    if total_delta >= 3 and ib_delta >= 1 and sot_delta <= 0:
        return True

    return False


def verify_sot_via_events(
    client: httpx.Client, fid: int, tid: int, tname: str
) -> int | None:
    """v10.31: Verify SOT count via events endpoint (real-time data).

    Counts events with type="Shot" and detail="Shot on Target" for
    the given team. Events are pushed in real-time, unlike the
    aggregated stats endpoint which lags 2-3 minutes.

    Returns real-time SOT count, or None if verification fails.
    Costs 1 API credit per call.
    """
    global events_verify_calls_today

    _reset_events_verify_daily()

    if events_verify_calls_today >= EVENTS_VERIFY_DAILY_BUDGET:
        log.info(
            f"  EVENTS VERIFY: budget exhausted "
            f"({EVENTS_VERIFY_DAILY_BUDGET}/day), skipping"
        )
        return None
    ...
    try:
        ...
        events = data.get("response", [])
        events_verify_calls_today += 1

        # Count SOT for this team from shot events
        events_sot = 0
        for ev in events:
            if (ev.get("type") == "Shot"
                    and ev.get("detail") == "Shot on Target"
                    and ev.get("team", {}).get("id") == tid):
                events_sot += 1

        # Cache result
        events_verified_sot[(fid, tid)] = {
            "sot": events_sot, "ts": time.time()
        }

        log.info(
            f"  EVENTS VERIFY: {tname} F{fid} - "
            f"events SOT={events_sot} "
            f"(call #{events_verify_calls_today}/{EVENTS_VERIFY_DAILY_BUDGET} today)"
        )

        return events_sot

    except Exception as e:
        log.warning(f"  EVENTS VERIFY FAILED: {tname} F{fid}: {e}")
        return None
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    # v9.5: Remove from monitoring when past window
    # v10.13.2: Extended window — keep if SOT-accelerating or best_sot >= 2
    best_sot = get_fixture_best_sot(fid)
    sot_accel = fid in accelerating_fixtures or fid in pressure_accelerating
    is_extended = sot_accel or best_sot >= 2
    # v10.20: Always use extended max (don't drop fixtures at 61')
    hard_max = EXTENDED_MAX
    if minute > hard_max:
        fast_monitored.discard(fid)
        expire_fast_sot(fid)
    ...
    for tid, tname in ((home_tid, home["name"]), (away_tid, away["name"])):
        ...
        # SOT cannot exceed total shots; possession=0 + xG=N/A = incomplete data
        # v10.19: Also flag when ALL core shot stats are 0 (API returned no data)
        _data_corrupt = False
        if sot > total_shots and total_shots > 0:
            _data_corrupt = True
            log.warning(
                f"  DATA CORRUPT: {tname} F{fid} {minute}' — "
                f"SOT({sot}) > Total Shots({total_shots}), skipping signal"
            )
        if sot >= 3 and total_shots == 0:
            _data_corrupt = True
            log.warning(
                f"  DATA CORRUPT: {tname} F{fid} {minute}' — "
                f"SOT({sot}) but Total Shots=0, skipping signal"
            )
        ...
        # === v10.31: Events-based SOT verification (API lag bypass) ===
        # If stats show InBox/shots jumping but SOT frozen, verify via events.
        # Events endpoint is real-time; stats endpoint lags 2-3 min.
        # This directly fixes the Liverpool problem (stats SOT=2, real SOT=5).
        if detect_stats_lag(sot, total_shots, shots_inside_box, state, fid, tid):
            log.info(
                f"  LAG DETECTED: {tname} F{fid} {minute}' - "
                f"SOT={sot} InBox={shots_inside_box} Shots={total_shots} "
                f"(prev SOT={state.get('last_sot', '?')} "
                f"prev IB={state.get('last_shots_inside_box', '?')} "
                f"prev Shots={state.get('last_total_shots', '?')}), "
                f"verifying via events..."
            )
            events_sot = verify_sot_via_events(client, fid, tid, tname)
            if events_sot is not None and events_sot > sot:
                log.info(
                    f"  SOT CORRECTED: {tname} F{fid} {minute}' - "
                    f"stats SOT={sot} -> events SOT={events_sot} "
                    f"(API lag bypassed, +{events_sot - sot} SOT recovered)"
                )
                sot = events_sot
            elif events_sot is not None:
                log.info(
                    f"  SOT VERIFIED: {tname} F{fid} {minute}' - "
                    f"events confirms SOT={events_sot} (no lag, stats correct)"
                )
...
def main():
    log.info("=" * 60)
    log.info("")
    log.info("v10.31 CHANGES (API lag bypass via events):")
    log.info("  EVENTS-BASED SOT VERIFICATION: when stats endpoint lags (InBox jumping")
    log.info("  but SOT frozen), calls /fixtures/events to get real-time SOT count.")
    log.info("  Lag detection: InBox +2 with SOT flat, or Shots +3 + InBox +1 with SOT flat.")
    log.info("  Budget: max 5 events calls/day (1 credit each). 3-min cooldown per team.")
    log.info("  This fixes the Liverpool case: stats showed SOT=2 while real SOT=5.")
    log.info("")
    log.info("Football Bot v10.31 — Fix gps NameError in SOT burst combo calc")
    log.info("=" * 60)
