...
known_league_team_ids: set[int] = set()  # accumulates team IDs from tracked leagues across days
...
def fetch_daily_active_hours(client: httpx.Client) -> bool:
    ...
    try:
        ...
        # Uses persistent known_league_team_ids (accumulates across days).
        # If cache is too small (<50 teams), accept all — can't filter reliably.
        # (20 leagues have ~400 teams; 50 means we've seen at least a few leagues)
        global active_friendly_fixtures
        MIN_TEAM_CACHE = 50
        if len(kickoff_hours) < FRIENDLY_THRESHOLD and friendly_candidates:
            added = 0
            skipped = 0
            use_team_filter = len(known_league_team_ids) >= MIN_TEAM_CACHE
            for f, hour, name in friendly_candidates:
                if not use_team_filter or (
                    ...
                    skipped += 1
            if not use_team_filter:
                filter_note = f" (team cache small: {len(known_league_team_ids)}<{MIN_TEAM_CACHE}, all accepted)"
            else:
                filter_note = ""
            log.info(
