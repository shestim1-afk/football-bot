...
def classify_signal(sot: int, state: dict | None, current_minute: int,
                  pressure_building: bool = False) -> tuple[str | None, str, float]:
    # SOT < 2: NO signal regardless of pressure
    if sot < 2:
        return None, "", 0.0

    # SOT == 2: only if pressure is building (v9.9)
    if sot == 2 and not pressure_building:
        return None, "", 0.0
    ...
    # v9.9: Pressure-triggered SOT=2 always classifies as PRESSURE (no bump)
    if sot == 2 and pressure_building:
        return "PRESSURE", "", 0.0
...
def detect_pressure_building(tstats: dict, state: dict | None, minute: int) -> tuple[bool, str]:
    ...
    if not state or not state.get("last_minute", 0) > 0:
        return False, ""
    try:
        da = int(tstats.get("Dangerous Attacks", "0"))
        ts = int(tstats.get("Total Shots", "0"))
    except (ValueError, TypeError):
        return False, ""
...
def save_outcome(entry: dict) -> None:
    """Append a resolved signal outcome to the JSONL file."""
    try:
...
def check_signal_outcomes(fixture: dict) -> None:
    """v9.8: Check pending signals for this fixture.

    Two-track system:
      outcome_15min: HIT if goal within 15 game min, MISS after window expires
      outcome_full:  HIT if goal at any point before match ends, MISS at FT
    Entry is 'resolved' only when BOTH are decided (match must end).
    """
    fid = fixture["fixture"]["id"]
    status = fixture["fixture"]["status"]["short"]
    minute = fixture["fixture"]["status"].get("elapsed", 0) or 0
    ...
    for entry in signal_outcomes:
        ...
        if goals_since_signal > 0:
            ...
            if entry["outcome_15min"] is None:
                if mins_to_goal <= OUTCOME_WINDOW_MINUTES:
                    entry["outcome_15min"] = "HIT"
                    entry["goal_minute_15"] = minute
                    log.info(
                        f"  OUTCOME 15min HIT: {entry['team_name']} scored at {minute}' "
                        f"(+{mins_to_goal}') [{entry['league']}]"
                    )
                else:
                    # Goal came after 15-min window already expired
                    entry["outcome_15min"] = "MISS"  # already expired earlier
                    log.info(
                        f"  OUTCOME 15min MISS (late goal): {entry['team_name']} "
                        f"scored at {minute}' (+{mins_to_goal}') [{entry['league']}]"
                    )

            if entry["outcome_full"] is None:
                entry["outcome_full"] = "HIT"
                entry["goal_minute_full"] = minute
                if entry["outcome_15min"] == "MISS":
                    log.info(
                        f"  OUTCOME FULL HIT: {entry['team_name']} scored at {minute}' "
                        f"(+{mins_to_goal}' after signal, outside 15' window) "
                        f"[{entry['league']}]"
                    )
                else:
                    log.info(
            ...
            # Both decided if match also ended
            if status not in LIVE_STATUSES:
                entry["resolved"] = True
                save_outcome(entry)
            ...
            # No goal yet
            # Check 15-min window
            if entry["outcome_15min"] is None and mins_since >= OUTCOME_WINDOW_MINUTES:
                entry["outcome_15min"] = "MISS"
                log.info(
                    f"  OUTCOME 15min MISS: {entry['team_name']} no goal in {OUTCOME_WINDOW_MINUTES}' "
                    f"(signal at {entry['game_minute']}', now {minute}') "
                    f"[{entry['league']}] (still watching for full-match)"
                )
            ...
            if status not in LIVE_STATUSES:
                if entry["outcome_full"] is None:
                    ...
                    log.info(
                        f"  OUTCOME FULL MISS: {entry['team_name']} no goal "
...
def log_outcome_summary() -> None:
    ...
    Shows both 15-min (imminent) and full-match (eventual) hit rates,
    broken down by tier and by signal number.
    """
    ...
    m15 = sum(1 for e in resolved if e.get("outcome_15min") == "MISS")
    hf = sum(1 for e in resolved if e.get("outcome_full") == "HIT")
    mf = sum(1 for e in resolved if e.get("outcome_full") == "MISS")

    r15 = h15 / total * 100 if total else 0
    rf = hf / total * 100 if total else 0

    log.info("")
    log.info(f"=== SIGNAL OUTCOME SUMMARY: {total} signals ===")
    log.info(f"  15-min (imminent): {h15}H/{m15}M ({r15:.0f}%)")
    log.info(f"  Full-match:       {hf}H/{mf}M ({rf:.0f}%)")
    if h15 < hf:
        late = hf - h15
        log.info(f"  Late goals (after 15' window): {late}")
    ...
    for tier in ("VERY STRONG", "STRONG", "PRESSURE"):
        ...
        if hits_15:
            avg_15 = sum(e["goal_minute_15"] - e["game_minute"] for e in hits_15) / len(hits_15)
        goal_info = f", avg +{avg_15:.0f}' to goal" if avg_15 else ""
        log.info(
            f"  {tier}: 15min {t_h15}/{t_total} ({t_h15/t_total*100:.0f}%){goal_info} | "
            f"full {t_hf}/{t_total} ({t_hf/t_total*100:.0f}%)"
        )
    ...
    for sig_label, sig_filter in [("1st signal", lambda e: e.get("sig_num") == 1),
        ...
        g_total = len(group)
        avg_sot = sum(e.get("sot", 0) for e in group) / g_total
        log.info(
            f"  {sig_label}: 15min {g_h15}/{g_total} ({g_h15/g_total*100:.0f}%) | "
            f"full {g_hf}/{g_total} ({g_hf/g_total*100:.0f}%) | avg SOT: {avg_sot:.1f}"
        )

    # v9.9: Pressure-triggered vs normal signals
    pressure_sigs = [e for e in resolved if e.get("pressure_triggered")]
    normal_sigs = [e for e in resolved if not e.get("pressure_triggered")]
    if pressure_sigs:
        p_h15 = sum(1 for e in pressure_sigs if e.get("outcome_15min") == "HIT")
        p_hf = sum(1 for e in pressure_sigs if e.get("outcome_full") == "HIT")
        p_t = len(pressure_sigs)
        log.info(
            f"  PRESSURE-TRIGGERED (SOT=2): 15min {p_h15}/{p_t} ({p_h15/p_t*100:.0f}%) | "
            f"full {p_hf}/{p_t} ({p_hf/p_t*100:.0f}%)"
        )
    if normal_sigs:
        n_h15 = sum(1 for e in normal_sigs if e.get("outcome_15min") == "HIT")
        n_hf = sum(1 for e in normal_sigs if e.get("outcome_full") == "HIT")
        n_t = len(normal_sigs)
        log.info(
            f"  NORMAL (SOT>=3): 15min {n_h15}/{n_t} ({n_h15/n_t*100:.0f}%) | "
            f"full {n_hf}/{n_t} ({n_hf/n_t*100:.0f}%)"
        )
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    for tid, tname in ((home_tid, home["name"]), (away_tid, away["name"])):
        ...
        # v9.9: Parse pressure indicators (already in the stats response, zero extra cost)
        try:
            dangerous_attacks = int(tstats.get("Dangerous Attacks", "0"))
        except (ValueError, TypeError):
            dangerous_attacks = 0
        try:
        ...
        # v9.9: Detect pressure building (always, for logging awareness)
        pressure_is_building = False
        pressure_desc = ""
        if sot < 3:  # only check pressure when SOT hasn't already triggered
            pressure_is_building, pressure_desc = detect_pressure_building(tstats, state, minute)
            if pressure_desc:  # always log pressure state (even if below threshold)
                shots_off = total_shots - sot
                log.info(
                    f"  PRESSURE RADAR: {tname} SOT={sot} | {pressure_desc} | "
                    f"{'BUILDING' if pressure_is_building else 'below threshold'}"
                )

        tier, trend, sot_rate = classify_signal(sot, state, minute, pressure_building=pressure_is_building)
        ...
        pressure_note = ""
        if pressure_is_building:
            pressure_note = f"\n\n\u26a1 PRESSURE BUILDING (SOT={sot} early trigger)\n{pressure_desc}"
        msg = (
        ...
        if send_telegram(client, msg):
            log.info(
                ...
                f"{sig_label} signal{'  PRESSURE-TRIGGERED' if pressure_is_building else ''})"
            )
        ...
        # v9.8: Record signal for outcome tracking (dual: 15-min + full-match)
        ...
        signal_outcomes.append({
            ...
            "is_home": is_home_sg,
            "outcome_15min": None,  # HIT/MISS within 15 game min
            "outcome_full": None,   # HIT/MISS by end of match
            "goal_minute_15": None, # minute of goal if 15-min HIT
            "goal_minute_full": None, # minute of goal if full-match HIT
            "sig_num": sig_num,
            "pressure_triggered": pressure_is_building,
            "resolved": False,      # True when both outcomes are decided
        })
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v9.9 — Pressure building detection + conditional SOT=2 gate")
    log.info("=" * 60)
    ...
    log.info("")
    log.info("v9.9 CHANGES:")
    log.info("  Pressure radar: logs dangerous attacks rate + shots off target (zero API cost)")
    log.info("  Conditional SOT=2 gate: fires at SOT=2 if pressure is building")
    log.info("    Thresholds: DA-rate>=0.5/min, off-target>=3, DA>=10")
    log.info("  Pressure-triggered signals tracked separately in outcomes for backtesting")
    log.info("")
    log.info("v9.8.1 CHANGES:")
    log.info("  Fixed dead code in log_outcome_summary() — now shows by-signal-number breakdown")
    log.info("  Added sig_num field to outcome records for accurate per-signal analysis")
    log.info("")
    log.info("v9.8 CHANGES:")
    log.info(f"  Dual outcome tracking: 15-min (imminent) + full-match (eventual)")
    log.info(f"  Persistence: outcomes saved to JSONL (survives restarts)")
    log.info(f"  Status line shows both hit rates + pending count")
    log.info("")
