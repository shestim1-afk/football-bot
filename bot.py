...
LEAGUE_IDS = {
    ...
    88: "Eredivisie", 203: "Super Lig", 310: "Austrian Bundesliga",  # 169 was Chinese Super League!
...
def get_budget_mode() -> str:
    if quota_remaining is None:
        return "UNKNOWN"
    # v10.1.2: Premium thresholds (7500 req/day)
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v10.1.2 — Goal Pressure Score + League ID Verification")
    log.info("=" * 60)
    ...
    global signal_outcomes, LEAGUE_IDS
    ...
    with httpx.Client(timeout=30.0) as client:
        # --- v10.1.2: Verify small-league IDs against API (1 credit) ---
        # Prevents another ID-169-style disaster. Only checks IDs not in the
        # "big 8" (39,140,78,135,61,2,3,94,88,203,848) which are universally correct.
        ...
        if verify_ids:
            try:
                vresp = client.get(
                    f"{API_BASE}/leagues",
                    params={"ids": ",".join(str(i) for i in verify_ids)},
                    headers={"x-apisports-key": API_KEYS[0]},
                )
                update_quota(vresp)
                if vresp.status_code == 200:
                    vdata = vresp.json().get("response", [])
                    api_map = {lg["league"]["id"]: lg["league"]["name"] for lg in vdata}
                    mismatches = []
                    for lid in sorted(verify_ids):
                        expected = LEAGUE_IDS[lid]
                        actual = api_map.get(lid, "??? (NOT FOUND)")
                        # Soft match: check key words overlap
                        expected_words = set(expected.lower().split())
                        actual_words = set(actual.lower().split())
                        overlap = expected_words & actual_words
                        if lid not in api_map:
                            mismatches.append(f"  ID {lid}: expected '{expected}' but API returned NOTHING (ID doesn't exist!)")
                        elif len(overlap) < min(2, len(expected_words)):
                            mismatches.append(f"  ID {lid}: expected '{expected}' but API says '{actual}' — REMOVED from tracking")
                        else:
                            log.info(f"  League ID OK: {lid} = {actual}")
                    if mismatches:
                        log.warning("LEAGUE ID VERIFICATION FAILED — removing wrong IDs:")
                        for m in mismatches:
                            log.warning(m)
                            # Extract ID from mismatch line
                            try:
                    ...
                    log.warning(f"  League verification API call failed (HTTP {vresp.status_code}), skipping")
            except Exception as e:
