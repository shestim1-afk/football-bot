...
def _build_red_card_block(
    ...
    try:
        if rc_events is None:
            # No events coverage for this fixture — keep the stats-based
            # count line (counts only, no player names).
...
def fetch_top_sot_players(
    ...
    if team_id in _cached and _goals_ok and _growth_ok:
        ...
        if _r:
            ...
            _last_top_sot_info = {"outcome": "ok", "listed_sot": _ev_sot_cached}
            return _r
...
def _build_market_block(
    ...
    """v10.80: CARDS & CORNERS block for the Telegram signal.

    Returns (block_text, extras_for_record). Fully defensive: any missing
    input degrades that market's section, never the signal send.
    Identical render path is reused by the live edit updates so the
    message shape stays byte-stable between edits.
    """
    extras: dict = {
    ...
    try:
        parts = [
            "",
            "\U0001f7e8\U0001f6a9 CARDS & CORNERS \u2014 v1 heuristic (labels from today)",
        ]
        book_tag = f" \u00b7 {book_name} PRE ref" if book_name else " \u00b7 PRE ref"

        # --- CARDS ---
        if cards_now is None:
            parts.append("\U0001f7e8 Cards: live count n/a on this feed")
        else:
            lam_c = None
            p_c = None
            ...
            if p_c is not None:
                extras["mkt_cards_fair_over"] = round(1.0 / p_c, 2) if p_c > 1e-9 else None
            extras["mkt_cards_lean"] = _mkt_lean(p_c)

            line_txt = f"line O/U {cards_line:g}" if cards_line is not None else "no book line"
            price_txt = ""
            if cards_ov or cards_un:
                price_txt = (
                    f"\nbook O{cards_line:g} @{cards_ov} / U{cards_line:g} @{cards_un}"
                    f"{book_tag}"
                )
            parts.append(
                f"\U0001f7e8 Cards: {cards_now} now \u00b7 {fouls_now if fouls_now is not None else '?'} fouls"
                f" \u00b7 {red_now or 0}R at {game_minute}' | {line_txt}{price_txt}"
            )
            if lam_c is not None:
                if p_c is not None:
                    fair_c = (1.0 / p_c) if p_c > 1e-9 else None
                    parts.append(
                        f"proj {lam_c:.1f} \u2192 P(over) {p_c:.0%} \u00b7 fair O "
                        f"{fair_c:.2f} | LEAN: {extras['mkt_cards_lean']}"
                    )
                else:
                    parts.append(f"proj {lam_c:.1f} \u00b7 no line \u2192 no lean")
        ...
        line_txt = f"line O/U {corners_line:g}" if corners_line is not None else "no book line"
        price_txt = ""
        if corners_ov or corners_un:
            price_txt = (
                f"\nbook O{corners_line:g} @{corners_ov} / U{corners_line:g} @{corners_un}"
                f"{book_tag}"
            )
        parts.append(
            f"\U0001f6a9 Corners: {corners_now} now at {game_minute}' | {line_txt}{price_txt}"
        )
        if p_n is not None:
            fair_n = (1.0 / p_n) if p_n > 1e-9 else None
            parts.append(
                f"proj {lam_n:.1f} \u2192 P(over) {p_n:.0%} \u00b7 fair O "
                f"{fair_n:.2f} | LEAN: {extras['mkt_corners_lean']}"
            )
        else:
            parts.append(f"proj {lam_n:.1f} \u00b7 no line \u2192 no lean")

        parts.append("Rule: bet only at live price \u2265 printed fair. Counts auto-update.")
        return "\n".join(parts), extras
    except Exception:
        return "", {}
...
def _build_odds_value_block(
    ...
    try:
        ...
        if odds_data and odds_data.get("over_odds"):
            ...
            _mkt = (
                f"market O{odds_data['over_line']:.1f} "
            ...
            if _mkt_live:
                ...
                if ev_pct >= 3.0:
                    ...
                    _mkt += f" \u2192 no edge (fair {be_any:.2f})"
                else:
                    _mkt += " \u2192 thin edge"
            lines.append(_mkt)
        else:
            lines.append("market: no price captured (quota/coverage)")

        # --- fair side (fair = break-even at the user's book) ---
        _fair = f"fair O{any_line:.1f} {be_any:.2f} ({p_any:.0%})"
        if be_team is not None:
            _fair += (
                f" | {team_name} to score (FT) {be_team:.2f} "
                f"({p_team:.0%})"
            )
        lines.append(_fair)
        ...
        if (
            ...
            lines.append(
                f"BTTS Yes @{float(_btts_odds):.2f} "
                f"vs fair {1.0 / float(_p_btts):.2f}"
            )

        # the rule — identical line every time so the reflex forms
        if odds_data and odds_data.get("over_odds") and not _mkt_live:
            lines.append(
                "shown price is PRE-match \u2014 bet your book's LIVE price "
                "\u2265 break-even"
            )
        else:
            lines.append("rule: bet only at live price \u2265 break-even")
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    for tid, tname in ((home_tid, home["name"]), (away_tid, away["name"])):
        ...
        if _prev_goals is not None and team_goals > _prev_goals:
            ...
            _register_goal_sot_pending(fid, tid, team_goals, _prev_goals)
        ...
        msg = (
            ...
            f"Opponent SOT: {opponent_sot} | Opponent xG: {opponent_xg}{_bc_fresh}\n\n"
        ...
        msg += (
            f"\n\n\U0001f4c8 Goal Projection (xG {'from API' if _goal_pred['xg_source'] == 'api' else 'est. from SOT'}{_cal_note})"
        ...
        if _ft.get("p_team") is not None:
            ...
            msg += (
                f"\n\n\U0001f3c6 FT PREDICTION (live model{_rc_adj_note})\n"
                f"{tname} {_ft['p_team']:.0%} | Draw {_ft['p_draw']:.0%} | "
                f"{_opp_name_ft} {_ft['p_opp']:.0%}"
            )
