...
def get_stats_interval(budget_mode: str) -> int:
    ...
    """
    # v10.5.3: Aggressive polling — 7500 credits, prioritize speed
...
def get_sot_based_interval(fid: int, base_interval: int) -> int:
    """v10.5.3: Aggressive pressure-aware polling interval.

    v10.5.3: Tightened all tiers for faster signal delivery.
    v10 adds PRESSURE ACCELERATION as a top-priority tier:
      v10 NEW: Pressure-accelerating (GPS rising, 2+ deltas): 60s
      v10 NEW: GPS >= BUILDING (40) and unsignaled:              60s
      v9.7:   SOT-accelerating (SOT increased last poll):    60s
      v9.7:   SOT >= 3:                                     60s
              SOT == 2 + fast window:                        90s
              SOT == 2:                                     120s
              SOT == 1:                                     base interval
    ...
    # v9.7: SOT acceleration tier — still top priority
        ...
        interval = 60
    # v10.5.3: GPS-based acceleration — GPS >= BUILDING gets 60s
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v10.5.3 — Data Quality + Big Chances")
    log.info("=" * 60)
    ...
    log.info("  FASTER POLLING: Base 150s->90s, new GPS-based 60s tier")
    log.info("    — GPS >= 40 (BUILDING) now triggers 60s polling")
    log.info("    — SOT>=3: 90s->60s, SOT=2+fast: 120s->90s, SOT=2: 180s->120s")
    log.info("    — Window: 1'-85' (was 5'-85')")
    ...
    log.info("  POLLING: base 90s (was 150s), discovery 900s (NORMAL mode)")
