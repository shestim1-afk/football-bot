...
LEAGUE_IDS = {
    ...
    848: "Conference League", 357: "First League (Bulgaria)", 172: "First League (Bulgaria)", 94: "Primeira Liga",
...
def poll_event_fast_lane(client: httpx.Client) -> None:
    ...
    """
    global _event_fast_lane_fid, _event_fast_lane_last, _event_fast_lane_credits_today
    now = time.time()
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    for tid, tname in ((home_tid, home["name"]), (away_tid, away["name"])):
        ...
        if _impossible_sot:
            if _excess_sot >= 5:
                ...
                _track_blocked_candidate(
                    ...
                    gps, sot, ib_ratio, sh, sa, is_home_team,
                )
                tier = None
                trend = ""
                sot_rate = 0.0
        ...
        # v10.49 SPEED FIX (cont.): fetch Top SOT players AFTER the send.
        # Cached per fixture+team — repeat signals for the same fixture cost
        # 0 extra API credits. First fetch costs 1 credit, ~0.5-2s AFTER the
        # signal is already in Telegram.
        ...
        if top_sot_players:
            ...
            if _send_ok:
                send_telegram(
                    client,
                    f"\U0001f3af Top SOT — {tname}: {top_sot_str}\n"
                    f"({home['name']} {sh} - {sa} {away['name']}, {minute}')"
                )
