...
def find_candidates(fixtures: list[dict]) -> list[tuple[int, int, int]]:
    ...
    for fixture in fixtures:
        ...
        if minute < (MINUTE_MIN - PRE_WINDOW_MINUTES) or minute > MINUTE_MAX:
            continue
...
def do_discovery(client: httpx.Client) -> bool:
    ...
    log.info(f"  -> {len(candidates)} team-candidate(s) ({MINUTE_MIN - PRE_WINDOW_MINUTES}-{MINUTE_MAX}', signal from {MINUTE_MIN}+)")
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v10.16 — EOD date-window fix (include yesterday's signals at midnight boundary)")
    log.info("=" * 60)
    ...
    log.info(f"  Signal window: {MINUTE_MIN}-{MINUTE_MAX}' | Pre-window monitoring: {MINUTE_MIN - PRE_WINDOW_MINUTES}-{MINUTE_MIN - 1}' (baseline only, no signals)")
