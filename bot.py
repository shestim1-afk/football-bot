...
# v10.15: Pre-window monitoring — start polling 3 min before MINUTE_MIN
# to build a stats baseline. Signals still only fire at MINUTE_MIN+.
PRE_WINDOW_MINUTES = 3  # discover & poll from 18', signal from 21'
...
#   - BEFORE 20': signals fire from minute 10+ (early pressure)
#   - AFTER 61': signals fire until minute 75 (late pressure buildup)
EXTENDED_MIN = 10   # minimum minute for early-pressure signals
EXTENDED_MAX = 75   # maximum minute for late-pressure signals
...
def calculate_goal_pressure_score(
    ...
    if prev_state and prev_state.get("last_minute", 0) > 0:
        ...
        if sot_delta >= 2:
            ...
            gps_rising = len(gps_history) >= 1 and gps > gps_history[-1].get("gps", 0)
        ...
        accel_pts = min(accel_pts + sot_burst_pts, 20)  # v10.17: cap raised 15→20
        ...
        if goalless_count >= 4:
            ...
            goalless_pts = 1
        accel_pts = min(accel_pts + goalless_pts, 20)  # combined cap with burst
...
def find_candidates(fixtures: list[dict]) -> list[tuple[int, int, int]]:
    ...
    for fixture in fixtures:
        ...
        max_allowed = EXTENDED_MAX if is_extended else MINUTE_MAX
...
def get_sot_based_interval(fid: int, base_interval: int) -> int:
    """v10.13: Pressure-aware polling interval with acceleration tiers.

    v10.13 changes vs v10:
      - SOT == 2: reduced 180s → base interval (90s NORMAL)
      - SOT == 1: reduced base → base * 0.75 (67s NORMAL)
      - Unknown / 0 SOT (has state): reduced 1.5x → 1.0x base
      - Second-chance boost: if best SOT is 1-2 and minute > 40',
        poll at 60s to catch late-window triggers
    """
    best_sot = get_fixture_best_sot(fid)
    has_state = best_sot > 0 or any(f == fid for f, _ in team_state)
    ...
    elif fid in sot_burst_fixtures and not both_teams_signaled:
        interval = 60
    # v9.7: SOT acceleration tier — still top priority
    elif fid in accelerating_fixtures and not both_teams_signaled:
        interval = 60
    # v10.13: Second-chance boost — fast poll when SOT 1-2 and clock
    # is ticking (past 40'), every 60s to catch late triggers
        ...
        # v10.13: SOT=0 with state — was 1.5x, now 1.0x (same as base)
        budget = get_budget_mode()
        if budget in ("CAREFUL", "STRICT", "EMERGENCY"):
            interval = int(base_interval * 1.5)  # was 2.5x
        else:
            interval = base_interval  # was 1.5x
...
def do_discovery(client: httpx.Client) -> bool:
    ...
    for fid, (dh, da) in list(dead_fixtures.items()):
        ...
        sot_accel = fid in accelerating_fixtures or fid in pressure_accelerating
        is_extended = sot_accel or best_sot >= 2
        hard_max = EXTENDED_MAX if is_extended else MINUTE_MAX
    ...
    log.info(f"  -> {len(candidates)} team-candidate(s) ({MINUTE_MIN - PRE_WINDOW_MINUTES}-{MINUTE_MAX}', extended to {EXTENDED_MIN}-{EXTENDED_MAX} for SOT-accel, signal from {MINUTE_MIN}+)")
...
def check_telegram_commands(client: httpx.Client) -> None:
    """v10.17: Check for /stats, /stats3d, /stats7d, /matches commands from Telegram.

    Polls getUpdates with long polling disabled (quick check).
    /stats    = all-time stats from file
    /stats3d  = past 3 days
    /stats7d  = past 7 days
    /matches  = match list + team form (only when <10 games)
    ...
    try:
        ...
        for update in results:
            ...
            if text == "/stats":
                ...
                # v10.17: On-demand match list with team form (only if <10 games)
                _send_matches_command(client)
...
def _send_matches_command(client: httpx.Client) -> None:
    """v10.17: /matches command — match list + team form + top scorers.

    Only works when <10 games (too many = too long message + too many credits).
    Credit cost: 0 for match list, ~1 per team (stats) + ~1 per league (scorers).
    """
    if not todays_tracked_fixtures:
    ...
    if len(todays_tracked_fixtures) >= 10:
        send_telegram(
            ...
            f"/matches only available when <10 games to save credits & avoid spam."
        )
        return
    ...
    # --- League top scorers (1 credit per league) ---
    league_scorers: dict[int, list[dict]] = {}
    ...
    # --- Per-team season stats (1 credit per unique team) ---
    ...
    def _form_to_emoji(form_str: str) -> str:
        mapping = {"W": "\U0001f7e2", "D": "\U0001f7e1", "L": "\U0001f534"}
        return "".join(mapping.get(c, "") for c in form_str.upper() if c in mapping)

    def _short_name(name: str, max_len: int = 18) -> str:
        return name if len(name) <= max_len else name[:max_len - 1] + "\u2026"

    def _extract_team_line(ts: dict, tname: str) -> str:
        played = ts.get("fixtures", {}).get("played", {}).get("total", 0)
        if not played:
            return f"  {_short_name(tname)}: no data"
        gf_raw = ts.get("goals", {}).get("for", {}).get("average", None)
        ga_raw = ts.get("goals", {}).get("against", {}).get("average", None)
        if isinstance(gf_raw, dict):
            gf_raw = gf_raw.get("total", "?")
        if isinstance(ga_raw, dict):
            ga_raw = ga_raw.get("total", "?")
        try:
            gf = f"{float(gf_raw):.1f}" if gf_raw not in (None, "?") else "?"
            ga = f"{float(ga_raw):.1f}" if ga_raw not in (None, "?") else "?"
        except (ValueError, TypeError):
            gf = str(gf_raw)
            ga = str(ga_raw)
        form = ts.get("form", "")
        form_emoji = _form_to_emoji(form) if form else ""
        form_note = f" {form_emoji}" if form_emoji else ""
        league_pos = ts.get("league", {}).get("standings", 0)
        pos_str = f" #{league_pos}" if league_pos else ""
        return f"  {_short_name(tname)} ({played}g){pos_str}: {gf} GF | {ga} GA{form_note}"
    ...
    for fix in todays_tracked_fixtures:
        ...
        hts = team_stats_cache.get(htid, {})
        ats = team_stats_cache.get(atid, {})
        if not isinstance(hts, dict):
            hts = {}
        if not isinstance(ats, dict):
            ats = {}
        h_played = hts.get("fixtures", {}).get("played", {}).get("total", 0)
        a_played = ats.get("fixtures", {}).get("played", {}).get("total", 0)

        if not h_played and not a_played:
            continue

        lines.append(f"\u26bd {_short_name(league_name, 25)}")
        lines.append(f"  {_short_name(home['name'])} vs {_short_name(away['name'])}")
        lines.append(_extract_team_line(hts, home["name"]))
        lines.append(_extract_team_line(ats, away["name"]))
        lines.append("")
    ...
    log.info(f"v10.17: /matches sent ({len(todays_tracked_fixtures)} matches with form)"
)
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    for tid, tname in ((home_tid, home["name"]), (away_tid, away["name"])):
        ...
        # v10.13.2: Extended window — allow early signals (10'-20') if SOT-accelerating
        if minute < MINUTE_MIN:
            # Allow signals from EXTENDED_MIN if SOT-accelerating or best_sot >= 2
            fid_best_sot = get_fixture_best_sot(fid)
            fid_accel = fid in accelerating_fixtures or fid in pressure_accelerating
            if minute >= EXTENDED_MIN and (fid_accel or fid_best_sot >= 2):
                log.info(
...
def fetch_daily_active_hours(client: httpx.Client) -> bool:
    ...
    try:
        ...
        scheduled_window_entries = sorted(set(
            (wave * 60) + ((MINUTE_MIN - PRE_WINDOW_MINUTES) * 60)  # kickoff_ts + 18' (pre-window)
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v10.17 — SOT burst + goalless pressure priority")
    log.info("=" * 60)
    ...
    log.info("  Components: SOT(28) + InBox(22) + ShotVol(14) + xG(15) + Corners(4) + Poss(5) + ACCEL(20)")
    log.info(f"  Signal window: {MINUTE_MIN}-{MINUTE_MAX}' | Extended: {EXTENDED_MIN}-{EXTENDED_MAX}' for SOT-accelerating | Pre-window: {MINUTE_MIN - PRE_WINDOW_MINUTES}-{MINUTE_MIN - 1}' (baseline only)")
