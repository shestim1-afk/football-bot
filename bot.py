        ...
        if minute >= FRESHNESS_HARD_STOP:
            ...
            if _has_recency_data:
                ...
                if not _has_fresh_pressure:
                    # v10.40: BLOCK even CRITICAL if stale pressure at 61'+
                    # Data: LATE_OVERRIDE 17% 15-min vs CORE 31%.
                    # SOT>=3 alone isn't enough late-game — need fresh pressure.
                    # Exception: SOT>=5 still passes (extreme danger).
                    if tier == "CRITICAL" and sot < 5:
                        log.info(
                            f"  FRESHNESS BLOCK: {tname} CRITICAL at {minute}' — "
                            f"stale pressure, SOT={sot}<5, "
                ...
                if not _has_quality:
                    # v10.40: BLOCK even CRITICAL if IB<60% at 76'+
                    # Low in-box ratio late = hopeful long shots, not dangerous.
                    # Exception: SOT>=5 still passes (extreme danger).
                    if tier == "CRITICAL" and sot < 5:
                        log.info(
                            f"  FRESHNESS IB BLOCK: {tname} CRITICAL at {minute}' — "
                            f"IB={ib_ratio:.0%} < {int(FRESHNESS_IB_FLOOR*100)}%, SOT={sot}<5"
                        )
                        continue
                    else:
                        log.info(
...
def main():
    ...
    log.info("=" * 60)
    log.info("Football Bot v10.40 — Tightened late-game gates")
    log.info("=" * 60)
