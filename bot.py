...
def get_schedule_based_discovery_interval(budget_mode: str) -> int | None:
    ...
    if gap <= 0:
        # We're at/past the window entry — discover now
        return 10
...
def get_discovery_interval(
    ...
    v9.6.1: Uses the pre-computed schedule to wake up right when
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v9.6.1 — Schedule-based discovery (wake up at 20' entries, not blind polling)")
    log.info("=" * 60)
