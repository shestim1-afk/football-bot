...
def do_discovery(client: httpx.Client) -> bool:
    ...
    # v10.14: Use events-based resolution for finished fixtures.
    for f in cached_fixtures:
        if is_tracked_match(f):
            status = f["fixture"]["status"]["short"]
            if status not in LIVE_STATUSES:
                resolve_with_events(client, f)
            else:
...
def fetch_goal_events(client: httpx.Client, fixture_id: int) -> list[dict]:
    """v10.14: Fetch goal events for a fixture from /fixtures/events.

    Returns list of {"minute": int, "team_id": int, "type": str, "detail": str}
    for all goals (including own goals, penalties).
    """
    try:
        ...
        for ev in events:
            if ev.get("type") == "Goal":
                # Handle time.detail like "57' +2" -> 59
                time_obj = ev.get("time", {})
                minute = safe_int(str(time_obj.get("elapsed", 0) or 0))
                # Add extra time if present
                extra = time_obj.get("extra", 0)
                if extra:
                    minute += safe_int(str(extra))
                goals.append({
                    "minute": minute,
                    "team_id": ev.get("team", {}).get("id"),
                    "type": ev.get("detail", "Goal"),  # Normal Goal, Penalty, Own Goal
                    "player": ev.get("player", {}).get("name", ""),
                })
        return goals
    except Exception as e:
        log.warning(f"  Failed to fetch events for fixture {fixture_id}: {e}")
        return []


def resolve_with_events(client: httpx.Client, fixture: dict, goal_events: list[dict] | None = None) -> int:
    """v10.14: Resolve signals using actual goal event times.

    This fixes the time-window bug in check_signal_outcomes() where
    the 5/10/15 min windows were wrong when called after FT (used
    final match minute instead of actual goal minute).

    Uses /fixtures/events to get exact goal minutes.
    Falls back to check_signal_outcomes() if events fetch fails.
    """
    global signal_outcomes
    fid = fixture["fixture"]["id"]
    status = fixture["fixture"]["status"]["short"]

    pending = [e for e in signal_outcomes if e["fixture_id"] == fid and not e["resolved"]]
    if not pending:
        return 0

    # Only use events-based resolution for finished fixtures
    # For live fixtures, the existing check_signal_outcomes() works fine
    if status in LIVE_STATUSES:
        check_signal_outcomes(fixture)
        return sum(1 for e in signal_outcomes if e["fixture_id"] == fid and e.get("resolved"))

    # Fetch goal events if not provided
    if goal_events is None:
        goal_events = fetch_goal_events(client, fid)

    resolved_count = 0
    home_goals_final = fixture["goals"]["home"] or 0
    away_goals_final = fixture["goals"]["away"] or 0

    for entry in pending:
        team_id = entry["team_id"]
        is_home = entry["is_home"]
        final_goals = home_goals_final if is_home else away_goals_final
        goals_since = final_goals - entry["goals_at_signal"]
        sig_minute = entry["game_minute"]

        if goals_since > 0:
            # Team scored after signal — find exact goal minute from events
            team_goals_after = [
                g for g in goal_events
                if g["team_id"] == team_id and g["minute"] > sig_minute
            ]
            # Exclude own goals scored AGAINST this team
            # (own goals have type "Own Goal" but team_id is the scoring team's opponent)
            actual_goals = [
                g for g in team_goals_after
                if g["type"] not in ("Own Goal",)
            ]

            if actual_goals:
                first_goal_min = min(g["minute"] for g in actual_goals)
            else:
                # No non-own-goal events found, use fallback
                first_goal_min = None

            entry["outcome_full"] = "HIT"
            entry["goal_minute_full"] = first_goal_min or sig_minute
            ...
            if first_goal_min is not None:
                for window, outcome_key, minute_key in [
                    ...
                    if entry.get(outcome_key) is None:
                        delta = first_goal_min - sig_minute
                        if delta <= window:
                            entry[outcome_key] = "HIT"
                            entry[minute_key] = first_goal_min
                        else:
                            entry[outcome_key] = "MISS"
            else:
                # Fallback: team scored but we couldn't get goal times
                # At least mark full as HIT, leave time windows as-is if not yet set
                for outcome_key in ("outcome_5min", "outcome_10min", "outcome_15min"):
                    if entry.get(outcome_key) is None:
                        entry[outcome_key] = "UNKNOWN"

            log.info(
                f"  OUTCOME HIT: {entry['team_name']} goal at {first_goal_min or '?'}' "
                f"(signal {sig_minute}') GPS={entry.get('gps', '?')} [{entry['league']}]"
            )
        else:
            # No goals after signal
            ...
            log.info(
                ...
                f"(signal at {sig_minute}', GPS={entry.get('gps', '?')}) [{entry['league']}]"
            )
...
def resolve_stale_outcomes(client: httpx.Client) -> int:
    """v10.14: Resolve pending outcomes from finished fixtures using goal events.

    When the bot sleeps before matches end, pending outcomes stay unresolved.
    This function fetches finished fixtures by ID and resolves them using
    /fixtures/events for accurate goal-minute-based time windows.
    ...
    for i in range(0, len(fixture_ids), BATCH_SIZE_LIMIT):
        ...
        try:
            ...
            for f in fixtures:
                ...
                if status not in LIVE_STATUSES:
                    # v10.14: Use events-based resolution for finished fixtures
                    resolved_count += resolve_with_events(client, f)
                else:
                    # Live fixture — use existing real-time check
                    check_signal_outcomes(f)
        except Exception as e:
    ...
    if resolved_count > 0:
        log.info(f"Resolved {resolved_count} stale outcome(s) (events-based), {len(still_pending)} still pending")
    elif still_pending:
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    # v10.14: Use events-based resolution for finished fixtures
    status_early = fixture["fixture"]["status"]["short"]
    if status_early not in LIVE_STATUSES:
        resolve_with_events(client, fixture)
    else:
        check_signal_outcomes(fixture)
