...
FRESHNESS_HARD_STOP = 86        # 86'+ blocked unless event-confirmed SOT burst
        ...
        for update in results:
            ...
            if text == "/stats":
                ...
                if specific_date:
                    ...
                    send_telegram(client, "Generating EOD report (all data)...")
                    # eod_report.py defaults to today when no args — use large --days instead
                    cmd = ["python3", "eod_report.py", "--send", "--days", "365", "--quiet"]
                try:
        ...
        if minute >= FRESHNESS_HARD_STOP:
            ...
            if _has_recency_data:
                ...
                if not _has_fresh_pressure:
                    # v10.35: Context-aware CRITICAL — don't block, tag instead.
                    # SOT>=3 proves danger exists, but stale pressure is weaker.
                    # Data: GPS 100 stale signals went 0/4; fresh late signals hit.
                    # EW without freshness is still blocked (no SOT proof).
                    if tier == "CRITICAL":
                        stale_tag = (
                            f"\n\u26a0\ufe0f STALE CRITICAL — SOT\u22653 confirms danger "
                            f"but no fresh pressure in last 5min "
                            f"(SOT_5m=+{_sot_d5} GPS_chg={_gps_chg} accel={accel_count})"
                        )
                        log.info(
                            f"  FRESHNESS TAG (not block): {tname} CRITICAL at {minute}' — "
                            f"stale but SOT={sot} proves danger, tagging"
                        )
                    else:
                        log.info(
                            f"  FRESHNESS BLOCK: {tname} {tier} at {minute}' — "
                            f"no fresh pressure (SOT_5m=+{_sot_d5} GPS_chg={_gps_chg} accel={accel_count} GPS={gps:.0f})"
                        )
                        continue

                if not _has_quality:
                    # v10.35: CRITICAL also gets IB quality gate at 76'+,
                    # but tagged rather than blocked (SOT>=3 is strong evidence)
                    if tier == "CRITICAL":
                        stale_tag = (
                            f"\n\u26a0\ufe0f STALE CRITICAL — IB {ib_ratio:.0%} below 76'+ floor, "
                            f"SOT={sot} confirms danger but shot quality is low"
                        )
                        log.info(
                            f"  FRESHNESS IB TAG (not block): {tname} CRITICAL at {minute}' — "
                            f"IB={ib_ratio:.0%} < {int(FRESHNESS_IB_FLOOR*100)}% but SOT={sot}"
                        )
                    else:
                        log.info(
                            f"  FRESHNESS IB BLOCK: {tname} {tier} at {minute}' — "
                            f"76'+ requires IB>={int(FRESHNESS_IB_FLOOR*100)}% (got {ib_ratio:.0%})"
                        )
                        continue
...
def main():
    ...
    log.info("=" * 60)
    log.info("Football Bot v10.35 — Context-aware CRITICAL (tag stale, don't block)")
    log.info("=" * 60)
