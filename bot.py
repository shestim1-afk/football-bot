...
# --- Friendly matches fallback (v9.5.4) ---
# When few main-league matches are scheduled, also track friendlies.
# Only tracks friendlies where >=1 team plays in our 20 tracked leagues.
# STRICT: no fallback — if team cache is empty, no friendlies are added.
FRIENDLY_THRESHOLD = 5  # include friendlies if fewer main-league matches than this
...
def fetch_daily_active_hours(client: httpx.Client) -> bool:
    ...
    try:
        ...
        # If few main-league matches, include international club friendlies
        # ONLY if at least one team is in our 20 tracked leagues (strict — no fallback).
        global active_friendly_fixtures
        if len(kickoff_hours) < FRIENDLY_THRESHOLD and friendly_candidates:
            added = 0
            skipped = 0
            for f, hour, name in friendly_candidates:
                if (
                    ...
                    skipped += 1
            log.info(
                f"  Friendly fallback: {added} friendly(ies) added, "
                f"{skipped} lower-division skipped "
                f"(only {len(kickoff_hours) - added} main-league matches, "
                f"team cache: {len(known_league_team_ids)} IDs)"
            )
        else:
            if active_friendly_fixtures:
                log.info(f"  Clearing {len(active_friendly_fixtures)} friendly fixture IDs (enough main matches)")
        ...
        if is_recheck:
            # On re-checks, only WIDEN the window (never shrink)
            ...
            # First fetch of the day: set window directly
            dynamic_active_start = new_start
            dynamic_active_end = new_end
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v9.5.8 — First-signal-only on busy days, xG boost in ranking")
    log.info("=" * 60)
    ...
    log.info(f"  Friendly fallback: ON when <{FRIENDLY_THRESHOLD} main-league matches (20-league teams only, strict — no random friendlies)")
