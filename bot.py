...
def check_fastlane_shadow(fixture: dict, client: httpx.Client = None) -> None:
    ...
    for entry in fastlane_shadow:
        ...
        if goal_events is not None:
            ...
            if entry.get("outcome_full") == "HIT":
                log.info(
                    f"  SHADOW-HIT: {entry['team_name']} fast-lane shadow at "
...
# v10.57: value maps team_id -> [(player_name, sot_count, scored_flag), ...]
_player_sot_cache: dict[int, dict[int, list[tuple[str, int, bool]]]] = {}
...
def _filter_non_scorers(entries: list[tuple[str, int, bool]]) -> list[tuple[str, int]]:
    """v10.57: The Top SOT line shows players who have NOT scored yet.

    Sorted input is non-scorers by SOT desc first, then scorers. Returns only
    the non-scorers (the "scores next" candidates). Fallback: if EVERY player
    with a SOT already scored, return the full list (a hint beats no hint).
    """
    _non = [(n, c) for n, c, _scored in entries if not _scored]
    if _non:
        return _non
    return [(n, c) for n, c, _scored in entries]
...
def fetch_top_sot_players(
    ...
    headlines the second-best SOT player who has NOT scored. Full-list
    fallback only when every SOT player already scored.

    Returns list of (player_name, sot_count), up to max_players.
    Cached per fixture+team to avoid duplicate API calls; the cache is
    re-fetched after every goal (ordering depends on who scored).
    Each rebuild costs 1 API credit — only on signal sends after a goal.
    """
    ...
    if (
        team_id in _cached
        and _fixture_valid_goals.get(fixture_id, 0) <= _player_sot_cache_goals.get(fixture_id, 0)
    ):
        return _filter_non_scorers(_cached[team_id])[:max_players]
    ...
    try:
        ...
        # v10.57: entries carry the scorer flag (name, sot, scored) so the
        # non-scorer filter works on cache hits without re-parsing events.
        _fixture_data: dict[int, list[tuple[str, int, bool]]] = {}
        for tid, players in _sot_by_team_player.items():
            _goals = _goals_by_team_player.get(tid, {})
            _entries = [(p, c, _goals.get(p, 0) > 0) for p, c in players.items()]
            _entries.sort(key=lambda x: (x[2], -x[1], x[0]))
        ...
        # (own goals count — they change the score too; VAR-disallowed don't).
        _gc = sum(
            1 for ev in events
...
def main():
    ...
    global _player_sot_cache, _player_sot_cache_goals, _fixture_valid_goals
