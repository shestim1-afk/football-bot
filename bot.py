...
#   SOT: 0-35 | Inside Box: 0-27 | Shot Vol: 0-14 | xG: 0 (dead) | Corners: 0-4 | Accel: 0-28
...
def calculate_goal_pressure_score(
    ...
    v10.26 changes from v10.10:
      ...
      - xG was always 0/N/A from API. Points now go to components with actual data.
    ...
    # === 1. SOT component (0-35 points) ===
    # v10.42: boosted 28→35 (absorbed 7 xG points — xG always 0 from API).
    # Absolute SOT matters, but with diminishing returns.
    # SOT=0: 0, SOT=1: 5, SOT=2: 12, SOT=3: 22, SOT=4: 30, SOT=5+: 35
    sot_table = {0: 0, 1: 5, 2: 12, 3: 22, 4: 30}
    sot_pts = sot_table.get(min(sot, 5), 35)
    score += sot_pts
    components["sot"] = sot_pts

    # === 2. Shots inside box ratio (0-27 points) ===
    # v10.42: boosted 22→27 (absorbed 5 xG points — xG always 0 from API).
    # v10.10: boosted from 18 → 22 (absorbed 4 DA points)
    ...
    ib_pts = min(ib_ratio * 36, 27)  # 75%+ = full 27 pts
    score += ib_pts
    ...
    # === 4. xG — REMOVED in v10.42 ===
    # API-Football v3 NEVER returns xG data (always 0/N/A across all leagues).
    # The 18 points were redistributed: +7 SOT, +5 IB, +6 acceleration.
    ...
    if prev_state and prev_state.get("last_minute", 0) > 0:
        ...
        accel_pts = min(accel_pts + sot_burst_pts, 28)  # v10.42: cap 22→28 (absorbed 6 xG points)
        ...
        if goalless_count >= 4:
            ...
            goalless_pts = 1
        accel_pts = min(accel_pts + goalless_pts, 28)  # v10.42: cap 22→28 (absorbed 6 xG points)
...
def main():
    ...
    log.info("=" * 60)
    log.info("Football Bot v10.42 — Removed dead xG, redistributed 18pts to SOT/IB/accel")
    log.info("=" * 60)
    ...
    log.info("")
    log.info("v10.42 CHANGE (dead weight removal):")               
    log.info("  xG component removed (API always returns 0/N/A)")               
    log.info("  18pts redistributed: SOT 28→35 (+7), IB 22→27 (+5), Accel 22→28 (+6)")               
    log.info("  GPS scores will be higher for same pressure — thresholds unchanged")               
    log.info("")               
