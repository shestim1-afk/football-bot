...
# --- v9.5: Diversification tracking ---
# Fixtures that have already produced at least one signal.
# Used to prioritize unsignaled fixtures in ranking.
...
def get_fixture_sot_priority(fid: int) -> int:
    ...
    # Penalize already-signaled fixtures (they're lower priority)
    if is_signaled:
        base = int(base * 0.5)
...
def get_sot_based_interval(fid: int, base_interval: int) -> int:
    ...
    elif best_sot >= 2 and not is_signaled and is_fast_sot_active(fid):
    ...
    # Already-signaled fixtures: slower polling to free quota for new matches
    if is_signaled:
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    if best_current_sot >= 2 and fid not in signaled_fixtures:
    ...
    for tid, tname in ((home_tid, home["name"]), (away_tid, away["name"])):
        ...
        # --- v9.5: Signal generated! ---
        # Track this fixture as signaled for diversification
        ...
        if send_telegram(client, msg):
            log.info(
                ...
                f"{'NEW' if fid in signaled_fixtures else 'update'})"
            )
...
def check_monitored_stats(
    ...
    """Fetch statistics for monitored fixtures in batched API calls.
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v9.5 — Multi-Match Diversification")
    log.info("=" * 60)
    ...
    log.info("")
    log.info("v9.5 CHANGES:")
    log.info("  Window: 20'-80' (strict, no late tracking)")
    log.info("  Signal: SOT >= 3 mandatory (no signal for SOT < 3)")
    log.info("  Diversification: prioritize unsignaled fixtures")
    log.info(f"  Max monitored: dynamic up to {BATCH_SIZE_LIMIT} (was 1-2)")
    log.info("  Key rotation: round-robin across all healthy keys")
    log.info("  Signal format: SOT, xG, opponent, red cards, trend")
    log.info("  Scoreline filter: removed (0-0 = ranking bonus, not gate)")
    log.info("")
    log.info("PRIORITY HIERARCHY:")
    log.info("  1. SOT >= 3 (mandatory signal criterion)")
    log.info("  2. Different fixture not yet signaled")
    log.info("  3. 0-0 scoreline (ranking bonus)")
    log.info("  4. Higher SOT count")
    log.info("  5. Higher xG")
    log.info("  6. Minute window preference")
    log.info("")
    log.info("Architecture: discovery + batched SOT-smart stats")
    log.info(f"  Dead hours: {DEAD_HOUR_START}:00-{DEAD_HOUR_END}:00 UTC")
    ...
    log.info("  Signaled fixtures:    2x all intervals")
    log.info("=" * 60)
        ...
        while True:
            ...
            if fast_sot_until:
                ...
                for fid, until in fast_sot_until.items():
                    ...
                    sig_flag = "S" if fid in signaled_fixtures else "N"
                    parts.append(f"{fid}(SOT={sot},{sig_flag},{remaining}s)")
