...
def format_outcome_stats(entries: list[dict]) -> str:
    ...
    if resolved:
        ...
        for e in recent:
            ...
            lines.append(f"  {icon} {e.get('team_name','?')} | {e.get('league','?')} | GPS {e.get('gps','?')} | SOT {e.get('sot','?')} | 15m:{o15} full:{of}{gm_str}")
    else:
...
def check_telegram_commands(client: httpx.Client) -> None:
    """v10.11: Check for /stats command from Telegram.

    Polls getUpdates with long polling disabled (quick check).
    Only processes /stats command, ignores everything else.
...
def send_daily_summary(client: httpx.Client) -> None:
    """v10.11: Send daily match briefing with team season stats to Telegram.

    Fetches team statistics (avg goals, corners, yellow cards) and
    top scorer (goals + assists) for each team in today's matches.
    Called once per day when the bot first enters active hours.
    Credit cost: ~2 per team (team stats + player stats) = ~16 for 8 teams.
    """
    ...
    log.info(f"v10.11: Sending daily summary for {len(todays_tracked_fixtures)} match(es)...")
    ...
    try:
        ...
        # Track which teams we've already fetched (dedup across fixtures)
        team_stats_cache: dict[int, dict] = {}  # team_id -> stats
        team_scorer_cache: dict[int, dict] = {}  # team_id -> {name, goals, assists}
        ...
        for fix in todays_tracked_fixtures:
            ...
            # Fetch stats for each team (cache to avoid duplicate calls)
            for team in [home, away]:
                ...
                if tid not in team_stats_cache:
                    try:
                        ...
                        ts_resp = ts_data.get("response", {})
                        team_stats_cache[tid] = ts_resp
                    except Exception:
                        team_stats_cache[tid] = {}

                if tid not in team_scorer_cache:
                    try:
                        pl_data = api_get(
                            client, "/players",
                            {"team": tid, "season": season, "league": lid}
                        )
                        players = pl_data.get("response", [])
                        # Find top scorer
                        best = None
                        for p in players:
                            ...
                            if not stats:
                                continue
                            p_goals = stats[0].get("goals", {}).get("total", 0) or 0
                            p_assists = stats[0].get("assists", {}).get("total", 0) or 0
                            if best is None or p_goals > best["goals"] or (p_goals == best["goals"] and p_assists > best["assists"]):
                                best = {
                                    "name": p["player"].get("name", "?"),
                                    "goals": p_goals,
                                    "assists": p_assists,
                                }
                        team_scorer_cache[tid] = best
                    except Exception:
                        team_scorer_cache[tid] = None

                # Format team stats
                ts = team_stats_cache.get(tid, {})
                goals_for = ts.get("goals", {}).get("for", {}).get("average", "?")
                corners_avg = ts.get("corners", {}).get("average", "?")
                yellow_total = ts.get("cards", {}).get("yellow", {}).get("total", "?")
                played = ts.get("fixtures", {}).get("played", {}).get("total", 0)
                yellow_avg = f"{yellow_total / played:.1f}" if isinstance(yellow_total, (int, float)) and played > 0 else "?"

                lines.append(f"\U0001f4ca {tname} (S{season}, {played} games)")
                lines.append(f"   Goals: {goals_for}/gm | Corners: {corners_avg}/gm | YC: {yellow_avg}/gm")

                scorer = team_scorer_cache.get(tid)
                if scorer and (scorer["goals"] > 0 or scorer["assists"] > 0):
                    lines.append(
                        f"   \u2b50 {scorer['name']}: {scorer['goals']}G {scorer['assists']}A"
                    )
                lines.append("")

        # Send in chunks (Telegram 4096 char limit)
        msg = "\n".join(lines)
        # Split on double newlines (match boundaries) if too long
        if len(msg) <= 4000:
        ...
        log.info(f"v10.11: Daily summary sent ({len(todays_tracked_fixtures)} matches)")

    except Exception as e:
        log.warning(f"v10.11: Daily summary failed: {e}")
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v10.11 — stale outcome fix, /stats command, win rate tracking")
    log.info("=" * 60)
