    ...
    if is_first_signal_only_mode() and signaled_fixtures:
        ...
        if skipped:
            ...
            for fid in sig_skipped_fids:
                f = find_cached_fixture(fid)
                if f:
                    log.info(f"     SIG-DONE: {f['teams']['home']['name']} vs {f['teams']['away']['name']}")
        if sig_revived_fids:
            log.info(f"  -> Revived {len(sig_revived_fids)} signaled fixture(s) (score changed)")
            for fid in sig_revived_fids:
                f = find_cached_fixture(fid)
                if f:
                    log.info(f"     SIG-REVIVE: {f['teams']['home']['name']} vs {f['teams']['away']['name']}")
