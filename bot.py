...
# v10.15: Pre-window monitoring — start polling 12 min before MINUTE_MIN
# v10.19: Set to 12 to match EXTENDED_MIN (9' = 21-12).
# Extra polling cost is small; EARLY GATE (GPS≥75) prevents noise.
PRE_WINDOW_MINUTES = 12  # discover & poll from 9', signal from 21'
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    for tid, tname in ((home_tid, home["name"]), (away_tid, away["name"])):
        ...
        if sig_count >= 2:
            ...
            if sot_jump < 1 or (sot_jump < 2 and gps < GPS_CRITICAL):
                log.info(
                    f"  BLOCKED {tier}: {tname} - "
                    f"{sot} SOT (+{sot_jump} from last signal, need +1 w/ GPS>={GPS_CRITICAL} or +2) "
...
def main():
    ...
    log.info("  PRE_WINDOW 3→12 min (polling from 9' to build baseline)")
