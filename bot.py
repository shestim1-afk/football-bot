...
def log_outcome_summary() -> None:
    ...
    """
    # v10.18: Only count first signal per team per fixture
    resolved = [e for e in signal_outcomes if e["resolved"] and e.get("sig_num", 1) == 1]
    if not resolved:
        return
...
def format_outcome_stats(entries: list[dict]) -> str:
    ...
    resolved = [e for e in first_sig if e.get("resolved")]
    pending = [e for e in first_sig if not e.get("resolved")]

    lines = []
    lines.append(f"📊 SIGNAL PERFORMANCE ({len(first_sig)} total, 1st signal only)")
    lines.append("")
    ...
    if resolved:
        ...
        for e in recent:
            ...
            lines.append(f"  {icon} {e.get('team_name','?')} | {e.get('league','?')} | GPS {e.get('gps','?')} | SOT {e.get('sot','?')} | {trigger} | 15m:{o15} full:{of}{gm_str}")
    else:
...
def check_telegram_commands(client: httpx.Client) -> None:
    """v10.18: Check for /stats, /stats3d, /stats7d, /today, /matches commands.

    Polls getUpdates with long polling disabled (quick check).
    /stats    = all-time stats from file
    /stats3d  = past 3 days
    /stats7d  = past 7 days
    /today    = match list only (no form/scorers, only if <10 games)
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    for tid, tname in ((home_tid, home["name"]), (away_tid, away["name"])):
        ...
        # v10.18: Fetch top SOT player for this team (1 credit, only on signal)
        top_sot_str = get_top_sot_player(client, fid, tname)
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v10.18 — No auto daily summary, /today command-only")
    log.info("=" * 60)
