...
SCHEDULE_RECHECK_INTERVAL = 3 * 3600  # re-fetch every 3h if no matches found
...
def fetch_daily_active_hours(client: httpx.Client) -> bool:
    ...
    If no matches found, re-checks every 3 hours in case API adds fixtures later.
    ...
    # Already fetched today — but re-check every 3h if no matches were found
    if schedule_date == today_str and not (schedule_no_matches and (time.time() - schedule_last_fetch) > SCHEDULE_RECHECK_INTERVAL):
        return not schedule_no_matches
    
    if schedule_date == today_str and schedule_no_matches:
        log.info(f"Re-checking schedule for {today_str} (last fetch found 0 matches >3h ago)...")
