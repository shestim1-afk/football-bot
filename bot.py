...
#   - BEFORE 21': signals fire from minute 16+ (early pressure, v10.19: raised from 10 — data shows <16' is noise)
#   - AFTER 61': signals fire until minute 80 (late pressure buildup, v10.19: raised from 75)
EXTENDED_MIN = 16   # minimum minute for early-pressure signals
EXTENDED_MAX = 80   # maximum minute for late-pressure signals
...
def _format_stats_block(entries: list[dict], label: str) -> list[str]:
    ...
    if resolved:
        ...
        minute_buckets = [
            ("16-35'", lambda e: 16 <= e.get("game_minute", 0) <= 35),
            ("36-45'", lambda e: 36 <= e.get("game_minute", 0) <= 45),
            ("46-61'", lambda e: 46 <= e.get("game_minute", 0) <= 61),
            ("62-80'", lambda e: 62 <= e.get("game_minute", 0) <= 80),
        ]
        gps_rows = [
            ("GPS 55-64", lambda e: 55 <= e.get("gps", 0) < 65),
            ("GPS 65-74", lambda e: 65 <= e.get("gps", 0) < 75),
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v10.19 — Window gates + N/A protection + GPS×min cross-tab")
    log.info("=" * 60)
    ...
    log.info("  EXTENDED_MIN 10→16, EXTENDED_MAX 75→80 (tighter early, wider late)")
