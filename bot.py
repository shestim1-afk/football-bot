...
def send_daily_summary(client: httpx.Client) -> None:
    ...
    for fix in todays_tracked_fixtures:
        for team in [fix["teams"]["home"], fix["teams"]["away"]]:
            ...
            goals_against = ts.get("goals", {}).get("against", {}).get("average", "?")
            corners_avg = ts.get("corners", {}).get("average", "?")
            yellow_total = ts.get("cards", {}).get("yellow", {}).get("total", "?")
            yellow_avg = (f"{yellow_total / played:.1f}"
                          if isinstance(yellow_total, (int, float)) and played > 0
                          else "?")
            form = ts.get("form", "")
            form_str = f" | Form: {form}" if form and len(form) <= 10 else ""

            lines.append(
                f"{tname} ({played} games): "
                f"GF {goals_for}/gm GA {goals_against}/gm | "
