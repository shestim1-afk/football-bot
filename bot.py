...
def _build_market_block(
    ...
    def _compact(emoji: str, label: str, now: int | None,
        ...
        if lean == "OVER" and p_over > 1e-9:
            return (f"{emoji} {label}: {now} vs line {line:g} \u00b7 OVER "
                    f"\u2014 bet O only \u2265 {1.0 / p_over:.2f}")
        if lean == "UNDER" and p_over < 1.0 - 1e-9:
            return (f"{emoji} {label}: {now} vs line {line:g} \u00b7 UNDER "
                    f"\u2014 bet U only \u2265 {1.0 / (1.0 - p_over):.2f}")
        return f"{emoji} {label}: {now} vs line {line:g} \u00b7 NEUTRAL \u2014 no bet"

    try:
        book_tag = f" ({book_name} PRE lines)" if book_name else " (PRE lines)"
        parts = [
            "",
            "\U0001f7e8\U0001f6a9 CARDS & CORNERS \u2014 counts auto-update" + book_tag,
        ]
