...
def main():
    ...
    with httpx.Client(timeout=30.0) as client:
        ...
        while True:
            ...
            _mg_live = _v10_72_live_monitored_fids(now)
            _mg_hold = False
            if _mg_live:
                if midnight_guard["since"] == 0.0 and not midnight_guard["gave_up"]:
                    ...
                    log.info(
                        f"v10.72 MIDNIGHT GUARD: {len(_mg_live)} live monitored fixture(s) "
                        f"at window rollover — holding watch until FT "
                        f"(max {MIDNIGHT_GUARD_MAX // 60}m): {_mg_names}"
                    )
                    try:
                        send_telegram(
                            client,
