...
def get_discovery_interval(
    ...
    if has_candidates:
        ...
        # v10.6: Ultra-aggressive discovery — 7500 credits, prioritize speed
        return {
            "NORMAL": 600,
            "CAREFUL": 900,
            "STRICT": 1200,
...
def get_stats_interval(budget_mode: str) -> int:
    ...
    """
    # v10.6: Ultra-aggressive polling — 7500 credits, max speed
...
def get_sot_based_interval(fid: int, base_interval: int) -> int:
    """v10.6: Ultra-aggressive pressure-aware polling interval.

    v10.6: All tiers slashed for near-instant signal delivery.
    v10 adds PRESSURE ACCELERATION as a top-priority tier:
      v10.6:  Pressure-accelerating (GPS rising, 2+ deltas):     15s
      v10.6:  GPS >= BUILDING (40) and unsignaled:               20s
      v10.6:  SOT-accelerating (SOT increased last poll):      15s
      v10.6:  SOT >= 3:                                        20s
              SOT == 2 + fast window:                           30s
              SOT == 2:                                        30s
              SOT == 1:                                        base interval
    ...
    # v10.6: Pressure acceleration — catches pre-SOT pressure build (15s)
    if (fid in pressure_accelerating
            and not both_teams_signaled
            and best_sot < 3):
        interval = 15
    # v10.6: SOT acceleration tier — 15s for fastest detection
    elif fid in accelerating_fixtures and not both_teams_signaled:
        interval = 15
    # v10.6: GPS-based acceleration — GPS >= BUILDING gets 20s
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v10.6 — Ultra-Aggressive Polling")
    log.info("=" * 60)
    ...
    log.info("  ULTRA-AGGRESSIVE POLLING: Base 90s->30s, high-pressure tiers 60s->15-20s")
    log.info("    — Pressure/SOT-accelerating: 60s->15s | GPS>=40: 60s->20s | SOT>=3: 60s->20s")
    log.info("    — SOT=2: 120s->30s | Discovery: 900s->600s | Sleep cap: 60s->30s")
    log.info("    — Window: 1'-85'")
    ...
    log.info("  v10.6:   Pressure-accelerating (rate-based 2+ deltas):  15s")
    log.info("  v10.6:   SOT-accelerating (SOT increased last poll):   15s")
    log.info("  v10.6:   GPS >= BUILDING (40), unsignaled:           20s")
    log.info("  v10.6:   SOT >= 3:                                    20s")
    log.info("  v10.6:   SOT >= 1 + fast window:                     30s")
    log.info("  v10.6:   SOT == 2:                                    30s")
    log.info("  v10.6:   SOT == 1:                                    base (30s)")
    ...
    with httpx.Client(timeout=30.0) as client:
        ...
        while True:
            ...
            sleep_time = min(sleep_time, 30)  # v10.6: was 60, now max 30s sleep
