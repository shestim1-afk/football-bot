...
def fetch_daily_active_hours(client: httpx.Client) -> bool:
    ...
    try:
        ...
        # Pass 1: collect team IDs from tracked leagues, build kickoff list
        kickoff_hours = []
        friendly_candidates = []  # (fixture, hour, league_name)
        tracked_team_ids: set[int] = set()
        ...
        for f in all_fixtures:
            ...
            if lid in LEAGUE_IDS:
                ...
                # Collect team IDs from tracked leagues for friendly filtering
                tracked_team_ids.add(f["teams"]["home"]["id"])
                tracked_team_ids.add(f["teams"]["away"]["id"])
        ...
        # If few main-league matches, include international friendlies only
        # (at least one team must play in one of our 20 tracked leagues)
        # Edge case: if 0 main-league matches, tracked_team_ids is empty,
        # so accept all friendlies as fallback (can't filter without reference teams)
        global active_friendly_fixtures
        if len(kickoff_hours) < FRIENDLY_THRESHOLD and friendly_candidates:
            added = 0
            skipped = 0
            use_team_filter = bool(tracked_team_ids)
            for f, hour, name in friendly_candidates:
                if not use_team_filter or (
                    f["teams"]["home"]["id"] in tracked_team_ids
                    or f["teams"]["away"]["id"] in tracked_team_ids
                ):
                    ...
                    skipped += 1
            filter_note = "" if use_team_filter else " (no main matches, all friendlies accepted)"
            log.info(
                f"  Friendly fallback: {added} international friendly(ies) added, "
                f"{skipped} lower-division skipped{filter_note} "
...
def main():
    ...
    log.info(f"  Friendly fallback: ON when <{FRIENDLY_THRESHOLD} main-league matches (international only, team-filtered)")
