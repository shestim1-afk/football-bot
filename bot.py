...
def send_daily_summary(client: httpx.Client) -> None:
    ...
    # v10.15: Redesigned — group by match, emoji form, skip no-data teams
    def form_to_emoji(form_str: str) -> str:
        """Convert WWDLW to 🟢🟢🟡🔴🟢"""
    ...
    for fix in todays_tracked_fixtures:
        ...
        if not h_played and not a_played:
            continue  # Skip if neither team has data

        any_match_stats = True

        def extract_team_line(ts: dict, tname: str) -> str:
            played = ts.get("fixtures", {}).get("played", {}).get("total", 0)
            if not played:
                return f"  {short_name(tname)}: no data"
            ...
            # Form
            form = ts.get("form", "")
            form_emoji = form_to_emoji(form) if form else ""
            ...
            return f"  {short_name(tname)} ({played}g){pos_str}: {gf} GF | {ga} GA{form_note}"

        home_line = extract_team_line(hts, home["name"])
        away_line = extract_team_line(ats, away["name"])

        lines.append(f"\u26bd {short_name(league_name, 25)}")
        lines.append(f"  {short_name(home['name'])} vs {short_name(away['name'])}")
