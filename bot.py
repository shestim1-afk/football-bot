...
LEAGUE_IDS = {
    ...
    357: "First League (Bulgaria)", 283: "SuperLiga (Serbia)",
    210: "HNL (Croatia)", 345: "Czech First League",
    119: "Danish Superliga", 137: "Veikkausliiga (Finland)",
    191: "NB I (Hungary)",
}
...
def get_top_sot_player(client: httpx.Client, fixture_id: int, team_id: int) -> tuple[str | None, int, int | None]:
    """v10.2.1: Fetch per-player SOT from /players?fixture={id}.

    Returns (player_name, sot_count, player_id) for the player with the most SOT
    on the specified team. Returns (None, 0, None) on any failure.
    Costs 1 API credit per call.
    """
    try:
        data = api_get(client, "/players", {"fixture": fixture_id})
        top_name = None
        top_sot = 0
        top_pid = None
        for entry in data.get("response", []):
            ...
            if sot > top_sot:
                ...
                top_pid = entry.get("player", {}).get("id")
        return (top_name, top_sot, top_pid)
    except Exception as e:
        log.warning(f"  Player stats fetch failed for fixture {fixture_id}: {e}")
        return (None, 0, None)


def get_goal_scorers(client: httpx.Client, fixture_id: int, team_id: int) -> set[int]:
    """v10.2.1: Get player IDs who scored for a team from /fixtures/events.

    Returns set of player IDs. Costs 1 API credit per call.
    """
    try:
        ...
        for event in data.get("response", []):
            if (event.get("type") == "Goal"
                    and event.get("team", {}).get("id") == team_id):
                pid = event.get("player", {}).get("id")
                if pid:
                    scorers.add(pid)
        return scorers
    except Exception as e:
        log.warning(f"  Events fetch failed for fixture {fixture_id}: {e}")
        return set()
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    for tid, tname in ((home_tid, home["name"]), (away_tid, away["name"])):
        ...
        # --- v10.2.1: Fetch top SOT player (goalless) ---
        # Costs 1-2 extra credits per signal (negligible with 7500/day).
        # Graceful: if fetch fails, signal still sends without player info.
        player_line = ""
        try:
            top_player, top_player_sot, top_pid = get_top_sot_player(client, fid, tid)
            if top_player and top_player_sot > 0:
                if goals_now == 0:
                    # Team has 0 goals — player is definitely goalless
                    player_line = f"\n\n shootout: {top_player} ({top_player_sot} SOT, 0 goals)"
                elif top_pid:
                    # Team scored — check if THIS specific player scored
                    scorers = get_goal_scorers(client, fid, tid)
                    if top_pid not in scorers:
                        player_line = f"\n\n shootout: {top_player} ({top_player_sot} SOT, 0 goals)"
                    else:
                        player_line = f"\n\n Top SOT: {top_player} ({top_player_sot} SOT, scored)"
                else:
                    player_line = f"\n\n Top SOT: {top_player} ({top_player_sot} SOT)"
        except Exception as e:
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v10.2 — Premium Expansion (27 leagues, faster polling)")
    log.info("=" * 60)
