...
# v10.6: League tier classification for value-window signals
TOP_TIER_LEAGUES = {39, 140, 78, 135, 61}  # Top 5
SECOND_TIER_LEAGUES = {79, 40, 141, 136, 62, 144, 340, 337}  # 2nd div + smaller top leagues
CUP_LEAGUES = {2, 3, 848}  # European cups


def get_league_tier_tag(league_id: int) -> str:
    """v10.6: Return a tier label for signal value assessment."""
    if league_id in TOP_TIER_LEAGUES:
        return "TOP TIER"
    if league_id in SECOND_TIER_LEAGUES:
        return "2ND TIER"
...
def get_value_window_tag(sh: int, sa: int) -> str:
    """v10.6: Scoreline-based value assessment for live betting.

    0-0: HIGH VALUE - market hasnt priced in shot pressure yet
    1-0/0-1: GOOD VALUE - one goal, market partially repriced
    1-1/2-1/1-2: VALUE - open match, market expects more goals
    2-0/0-2: LOW VALUE - leading team may sit back
    """
    total = sh + sa
    diff = abs(sh - sa)
    if total == 0:
...
LEAGUE_IDS = {
    ...
    337: "Allsvenskan (Sweden)",  # v10.6 fix: was 332 (Super Liga)
    # --- European cups ---
...
# v10.5.3: 1'-85' window (catch earliest pressure buildup)
MINUTE_MIN = 1
MINUTE_MAX = 85
...
# --- v10.7: Goal-release tier tracking ---
# Tracks which fixtures had a signaled team score and what release
# tier was applied. Prevents repeated release actions each cycle.
# Tiers: "soft" (46-69'), "full" (70'+). First half goals = no release.
# Allows upgrade: "soft" -> "full" when match crosses 70'.
goal_released: dict[int, str] = {}  # fixture_id -> "soft" | "full"

# --- v10.8: Pre-match prediction cache (odds-value boost) ---
# Key: fixture_id -> {"home_pct": float, "away_pct": float, "draw_pct": float}
# Fetched once per fixture when it enters fast monitoring (1 credit via /predictions).
# Used in rank_candidate to boost 0-0 games where the pressuring team has
# higher implied odds (lower win %) = more betting value if pressure is building.
fixture_predictions: dict[int, dict] = {}
...
# get_goal_scorers removed in v10.3

def fetch_prediction_if_needed(client: httpx.Client, fixture_id: int) -> dict | None:
    """v10.8: Fetch pre-match prediction once per fixture (1 credit).

    Uses /predictions endpoint which returns win probability percentages.
    Cached in fixture_predictions dict - subsequent calls return cached data.
    Only fetches once per fixture lifetime.

    Returns dict with home_pct, away_pct, draw_pct or None on failure.
    """
    if fixture_id in fixture_predictions:
        return fixture_predictions[fixture_id]

    try:
        data = api_get(client, "/predictions", {"fixture": fixture_id})
        resp = data.get("response", [])
        if not resp:
            return None
        preds = resp[0].get("predictions", {})
        percents = preds.get("percent", {})
        home_pct = parse_pct(percents.get("home", "0%"))
        away_pct = parse_pct(percents.get("away", "0%"))
        draw_pct = parse_pct(percents.get("draw", "0%"))
        result = {"home_pct": home_pct, "away_pct": away_pct, "draw_pct": draw_pct}
        fixture_predictions[fixture_id] = result
        log.info(f"  ODDS VALUE: Fetched prediction for fixture {fixture_id}: "
                 f"home={home_pct:.0f}% draw={draw_pct:.0f}% away={away_pct:.0f}%")
        return result
    except Exception as e:
        log.warning(f"  ODDS VALUE: Prediction fetch failed for fixture {fixture_id}: {e}")
        return None


def parse_pct(pct_str) -> float:
    """v10.8: Parse percentage string like '45%' to float 45.0."""
...
def get_odds_value_boost(fixture: dict, team_id: int) -> tuple[int, str]:
    """v10.8: Calculate ranking boost for 0-0 games based on pre-match odds.

    Logic: In a 0-0 game, if the pressuring team has HIGHER implied odds
    (lower win probability), the market doesn't expect them to score.
    If our bot detects pressure building on this team, that is HIGH VALUE
    because the odds have not moved yet because the market is slow to react.

    Boost only applies when:
    - Score is 0-0 (no goals scored)
    - We have prediction data (fetched once, 1 credit)
    - Match is < 75' (after that, time decay reduces value)

    Boost scale (added to rank_candidate score):
    - Win% < 25% (big underdog, odds ~4.0+):    +150 points
    - Win% 25-35% (moderate underdog, ~2.8-4.0): +100 points
    - Win% 35-45% (slight underdog, ~2.2-2.8):   +60 points
    - Win% 45-55% (even match, ~1.8-2.2):        +25 points
    - Win% > 55% (favorite, odds <1.8):             0 points (no extra value)

    Returns (boost_points, description_string).
    """
    ...
    fid = fixture["fixture"]["id"]
    pred = fixture_predictions.get(fid)
    if not pred:
        return 0, ""

    # Get the pressuring team's win probability
    is_home = fixture["teams"]["home"]["id"] == team_id
    win_pct = pred["home_pct"] if is_home else pred["away_pct"]

    # Time decay: boost fades after 70'
    ...
    if win_pct < 25:
        boost = 150
        label = f"ODDS+{boost} (underdog {win_pct:.0f}%, 0-0 @ {minute}')"
    elif win_pct < 35:
        boost = 100
        label = f"ODDS+{boost} (mod.underdog {win_pct:.0f}%, 0-0 @ {minute}')"
    elif win_pct < 45:
        boost = 60
        label = f"ODDS+{boost} (slight.underdog {win_pct:.0f}%, 0-0 @ {minute}')"
    elif win_pct < 55:
        boost = 25
        label = f"ODDS+{boost} (even {win_pct:.0f}%, 0-0 @ {minute}')"
    else:
        boost = 0
        label = ""
...
# 6. Odds value (0-0 underdogs)  (v10.8: high-implied-odds 0-0 games boosted)
#
...
def rank_candidate(fixture: dict, team_id: int) -> int:
    ...
    # --- 6. v10.8: Odds-value boost for 0-0 underdogs ---
    # In 0-0 games, a pressuring team with higher implied odds (lower win%)
    # represents more betting value - the market has not priced in the pressure.
    # Uses cached prediction data (1 credit fetched once per fixture).
    odds_boost, odds_label = get_odds_value_boost(fixture, team_id)
    if odds_boost > 0:
        score += odds_boost
...
def get_discovery_interval(
    ...
    if has_candidates:
        ...
        # v10.7: Instant signal — discover new fixtures 2x faster
        return {
            "NORMAL": 300,
            "CAREFUL": 600,
            "STRICT": 900,
...
def get_stats_interval(budget_mode: str) -> int:
    ...
    """
    # v10.7: Instant signal mode — fastest possible delivery
...
def _best_fixture_gps(fid: int) -> float:
    """v10.5.3: Get the highest GPS score for any team in this fixture."""
    best = 0.0
    for (f, t), state in team_state.items():
        if f == fid:
            gps = state.get("last_gps", 0)
            if gps > best:
                best = gps
    return best
...
def classify_signal(
    ...
    if sot >= 1 and gps >= GPS_EARLY_WARNING:
        ...
        # v10.6: Bypass sustained gate if GPS>=60 AND SOT>=2 (strong evidence)
        if sustained_count < 1 and gps < GPS_CRITICAL and not (gps >= 60 and sot >= 2):
            return None, "", 0.0

        last_sot = state["last_sot"] if state else 0
        # v10.7b: GPS-based CRITICAL now requires momentum (accel_count >= 1)
        # Prevents stale high-GPS from accumulated stats over 70 min
        # (e.g. 6 SOT slowly accumulated) from triggering without real urgency.
        # SOT >= 3 still triggers CRITICAL unconditionally (proven v9.x safety net).
        if gps >= GPS_CRITICAL and accel_count < 1:
            # High GPS but zero momentum — not imminent, treat as EARLY WARNING
            if sot > last_sot or (gps >= 60 and sot >= 2):
        ...
        # v10.6: Also allow if GPS>=60 and SOT>=2 even without SOT increase
        if sot > last_sot or gps >= GPS_CRITICAL or (gps >= 60 and sot >= 2):
            return "EARLY WARNING", trend, sot_rate
...
STAT_ALIASES: dict[str, list[str]] = {
    ...
    # v10.5.1: Fixed xG field mapping — API returns 'expected_goals', not 'Expected Goals'
    "expected_goals":     ["expected_goals", "Expected Goals", "expectedGoals", "Expected goals", "xG"],
    # v10.5.1: goals_prevented available from API (opponent's keeper saves quality proxy)
    "goals_prevented":    ["goals_prevented", "Goals Prevented"],
    # v10.5.3: Big Chances — human-scored clear scoring opportunities (stronger than generic SOT)
    "big_chances":        ["big_chances", "Big Chances", "Big chances"],
}
...
def stat_available(tstats: dict, stat_key: str) -> bool:
    """Check if any candidate for a stat key exists in the raw API response.

    Used for data quality scoring — distinguishes "API returned 0" from
    "API didn't return this field at all" (broken data feed).
    """
    candidates = STAT_ALIASES.get(stat_key, [stat_key])
    return any(tstats.get(c) is not None for c in candidates)
...
def calculate_goal_pressure_score(
    ...
    big_chances: int = 0,  # v10.5.3: human-scored clear opportunities
    data_quality: float = 1.0,  # v10.5.3: 0.0-1.0, fraction of optional stats available
) -> tuple[float, str, dict]:
    """v10.5.3: Calculate 0-100 Goal Pressure Score.
    ...
    # === 4. xG (0-20 points) ===
    # v10.5.1: Raised from 15 to 20 max (absorbed DA points since DA is gone from API).
    # Quality of chances created. 1.0 xG = 20 pts, 0.5 xG = 10 pts.
    if xg_value is not None and xg_value >= 0:
        xg_pts = min(xg_value * 20, 20)
    else:
        xg_pts = 0.0
    score += xg_pts
    ...
    # v10.5.1: DA removed from API for many leagues (2. Bundesliga, Championship, etc.).
    # Points redistributed to xG (max 15->20) since xG is now reliably available.
    # DA still used if available (some leagues may still return it).
    if minute > 0 and dangerous_attacks > 0:
    ...
    # === 6.3. Big Chances (0-5 points) — v10.5.3 ===
    # Human-scored clear scoring opportunities. Stronger signal than generic SOT
    # because it's a judgment call on genuine goal-scoring chances, not just shot location.
    # Only contributes if the API actually provides this stat for the league.
    # Scale: 1=1pt, 2=2.5pts, 3=4pts, 4+=5pts
    bc_pts = 0.0
    if big_chances > 0:
        bc_pts = min(big_chances * 1.25, 5)
    score += bc_pts
    components["big_chances"] = round(bc_pts, 1)

    # === 6.5. SOT VELOCITY (0-8 points) — v10.5.2 ===
    # How FAST are shots on target accumulating? This is the user's request.
    # A team going 2→5 SOT in 3' is far more dangerous than 2→3 SOT in 10'.
    # Uses the BEST of (current-poll rate, windowed rate) for responsiveness.
    # Scale: 0.15/m=2pts, 0.25/m=4pts, 0.40/m=6pts, 0.60+/m=8pts
    sot_velocity_pts = 0.0
    best_sot_rate = 0.0
    if prev_state and prev_state.get("last_minute", 0) > 0:
        _mp = max(minute - prev_state["last_minute"], 1)
        _sd = sot - prev_state.get("last_sot", 0)
        if _sd > 0:  # only if SOT is actually increasing
            best_sot_rate = _sd / _mp
    # Also check windowed rate (from GPS history)
    if len(gps_history) >= 2:
        _wmin = minute - GPS_WINDOW_MINUTES
        for _entry in gps_history:
            if _entry.get("minute", 0) >= _wmin:
                _wm = max(minute - _entry["minute"], 1)
                _wr = (sot - _entry.get("sot", 0)) / _wm
                if _wr > best_sot_rate:
                    best_sot_rate = _wr
                break
    if best_sot_rate >= 0.60:
        sot_velocity_pts = 8   # rapid fire: 1 SOT per ~1.5 min
    elif best_sot_rate >= 0.40:
        sot_velocity_pts = 6   # very fast: 2 SOT per 5 min
    elif best_sot_rate >= 0.25:
        sot_velocity_pts = 4   # fast: 1 SOT per 4 min
    elif best_sot_rate >= 0.15:
        sot_velocity_pts = 2   # moderate: 1 SOT per ~7 min
    score += sot_velocity_pts
    components["sot_velocity"] = round(sot_velocity_pts, 1)

    # === 7. Possession bonus (0-3 points) ===
    # v10.5.1: Moved from end — low weight, just a tiebreaker.
    poss_pts = 0.0
    if possession > 0:
        poss_pts = min((possession - 50) / 50 * 3, 3) if possession > 50 else 0.0
    score += poss_pts
    components["possession"] = round(poss_pts, 1)

    # === 9. ACCELERATION component (0-15 points) — THE KEY DIFFERENTIATOR ===
    ...
    if prev_state and prev_state.get("last_minute", 0) > 0:
        ...
        accel_details = (
            ...
            + (f" SOTvel={best_sot_rate:.2f}/m" if best_sot_rate >= 0.15 else "")
        )
    ...
    # === 10. Data quality flag — v10.5.3 ===
    # Tracks what fraction of optional stats (xG, DA, possession, big_chances)
    # were actually available from the API. A signal built on 1/4 available
    # components is weaker than one where all data populated.
    components["data_quality"] = round(data_quality, 2)

    # Cap at 100 after all components (including possession)
    score = min(score, 100.0)

    # v10.5.3: Include data quality and big chances in description
    bc_desc = f" BC:{big_chances}({components['big_chances']}pts)" if big_chances > 0 else ""
    dq_desc = f" DQ:{data_quality:.0%}" if data_quality < 1.0 else ""
    desc = (
...
def record_pressure_poll(
    ...
    try:
        entry = {
            ...
            "gps_sot_vel": components.get("sot_velocity", 0),  # v10.5.2
            "gps_bc": components.get("big_chances", 0),  # v10.5.3
            "gps_accel": components.get("acceleration", 0),
            "gps_poss": components.get("possession", 0),
            "data_quality": components.get("data_quality", 1.0),  # v10.5.3
...
def find_candidates(fixtures: list[dict]) -> list[tuple[int, int, int]]:
    ...
    for fixture in fixtures:
        ...
        if status not in LIVE_STATUSES:
            continue
        minute = fixture["fixture"]["status"].get("elapsed", 0) or 0
        minute = corrected_minute(fixture, minute)  # v10.5.3
...
def cleanup_state(live_fixture_ids: set[int]):
    ...
    # v10.7: Clean goal-release state
    for fid in list(goal_released):
        if fid not in live_fixture_ids:
            del goal_released[fid]
    # v10.8: Clean prediction cache
    for fid in list(fixture_predictions):
        if fid not in live_fixture_ids:
            del fixture_predictions[fid]
...
def corrected_minute(fixture: dict, api_minute: int) -> int:
    """v10.5.3: Correct API-reported minute using kickoff time.

    API-Football's elapsed minute can lag behind real-time. Observed 9+ min lag
    at halftime — API served 45' while match was at 54'. This inflates per-minute
    rates in GPS (shots/min, SOT velocity, DA rate all divide by minute).

    Fix: calculate real elapsed from kickoff timestamp, use max(api, calculated).
    During HT/ET/BT: trust API (match clock is stopped).
    """
    status = fixture["fixture"]["status"]["short"]

    # During HT/ET/BT/P, match clock behaves differently — trust API
    if status in ("HT", "ET", "BT", "P"):
        return api_minute

    date_str = fixture["fixture"].get("date")
    if not date_str or api_minute <= 0:
        return api_minute

    try:
        kickoff = datetime.fromisoformat(date_str)
        now = datetime.now(timezone.utc)
        real_elapsed = (now - kickoff).total_seconds() / 60

        # Subtract halftime break if in second half
        if status == "2H" and real_elapsed > 50:
            real_elapsed -= 15  # standard halftime break

        # Cap at half limits (don't exceed 45 in 1H, 90 in 2H)
        cap = 45 if status == "1H" else 90
        real_match_min = min(max(0, round(real_elapsed)), cap)

        # Use the higher of API vs calculated
        corrected = max(api_minute, real_match_min)
        if corrected > api_minute:
            log.debug(f"  MINUTE FIX: API={api_minute}' -> corrected={corrected}' (status={status})")
        return corrected
    except (ValueError, TypeError):
        return api_minute
...
def is_fixture_monitorable(fixture: dict) -> bool:
    """v9.5: strict 85' max — no late tracking beyond 85'.
    v10.5.1: In first-signal-only mode, only skip if BOTH teams already signaled.
    This allows the 2nd team in a fixture to get their first signal
    (e.g., Hertha BSC with 9 SOT after Heidenheim already signaled)."""
    status = fixture["fixture"]["status"]["short"]
    if status not in LIVE_STATUSES:
        return False
    minute = fixture["fixture"]["status"].get("elapsed", 0) or 0
    if minute < MINUTE_MIN or minute > MINUTE_MAX:
        return False
    # v10.5.1: Only skip if BOTH teams in this fixture have already signaled
    fid = fixture["fixture"]["id"]
    if is_first_signal_only_mode():
        teams_signaled = sum(1 for (f, t) in signaled_teams if f == fid)
        if teams_signaled >= 2:
            return False  # both teams already got their signal — move on
    return True
...
def get_fixture_sot_priority(fid: int) -> int:
    ...
    # v10.7b: 0-0 early-game priority boost
    # Matches with no goals have higher betting value — the market hasn't
    # priced in pressure yet. Poll these faster to catch pressure buildup
    # BEFORE odds move. Boost scales with how early in the match we are.
    fixture = find_cached_fixture(fid)
    if fixture:
        sh = fixture["goals"]["home"] or 0
        sa = fixture["goals"]["away"] or 0
        if sh == 0 and sa == 0 and not both_signaled:
            minute = fixture["fixture"]["status"].get("elapsed", 0) or 0
            if minute < 35:
                base = min(base + 20, 100)  # early game 0-0: big boost
            elif minute < 55:
                base = min(base + 12, 100)  # mid-game 0-0: moderate boost
            elif minute < 70:
                base = min(base + 6, 100)   # late-mid 0-0: small boost

    return base


def get_sot_based_interval(fid: int, base_interval: int) -> int:
    """v10.7: Instant signal — minimum latency polling intervals.

    v10.7: Further slashed from v10.6. Signal fires the instant API
    returns new SOT/pressure data. Max delay ~10s (loop sleep cap).
    v10.7:  Pressure-accelerating (GPS rising, 2+ deltas):     10s
    v10.7:  SOT-accelerating (SOT increased last poll):      10s
    v10.7:  GPS >= BUILDING (40) and unsignaled:               12s
    v10.7:  SOT >= 3:                                        12s
            SOT == 2 + fast window:                           15s
            SOT == 2:                                        15s
            SOT == 1:                                        base interval
            Unknown / 0 SOT:                                 1.5x or 2x base
    """
    best_sot = get_fixture_best_sot(fid)
    has_state = best_sot > 0 or any(f == fid for f, _ in team_state)
    ...
    # v10.7: Pressure acceleration — catches pre-SOT pressure build (10s)
    if (fid in pressure_accelerating
            and not both_teams_signaled
            and best_sot < 3):
        interval = 10
    # v10.7: SOT acceleration tier — 10s for fastest detection
    elif fid in accelerating_fixtures and not both_teams_signaled:
        interval = 10
    # v10.7: GPS-based acceleration — GPS >= BUILDING gets 12s
    elif not both_teams_signaled and _best_fixture_gps(fid) >= GPS_BUILDING:
        interval = 12
    elif not has_state:
...
def do_discovery(client: httpx.Client) -> bool:
    ...
    # v10.5.1: In FIRST_SIGNAL_ONLY mode, only exclude fixtures where
    # BOTH teams already got a signal (not just one team).
    # This allows the 2nd team to get their first signal on busy days.
    if is_first_signal_only_mode() and signaled_fixtures:
        before = len(fixture_best_rank)
        fixture_best_rank = {
            fid: rank for fid, rank in fixture_best_rank.items()
            if sum(1 for (f, t) in signaled_teams if f == fid) < 2
        }
        skipped = before - len(fixture_best_rank)
        if skipped:
    ...
    # v10.8: Fetch predictions for newly monitored fixtures (odds-value boost)
    new_fids = merged - retained
    for fid in new_fids:
        if fid not in fixture_predictions:
            fetch_prediction_if_needed(client, fid)
...
def is_scoreline_blocked(is_home: bool, sh: int, sa: int, minute: int, sot: int = 0) -> tuple[bool, str]:
    """v10.5.1: Check if a signal should be blocked due to scoreline context.

    Rules (relaxed from v10.5 — user wants signals even with bigger GD when SOT is high):
    - Team leads by 3+ goals -> protecting lead (was 2+)
    - Match is past 82' -> too little time (was 78')
    - Team already has 4+ goals AND leads by 3+ -> sitting back
    - EXCEPTION: If SOT >= 5, never block (team is clearly still attacking)
    ...
    # v10.5.1: High SOT override — if team has 5+ SOT, they're STILL attacking
    if sot >= 5:
        return False, ""
    ...
    if minute >= 82:
        return True, f"too late ({minute}', <8 min remaining)"
    if tg >= 4 and lead >= 3:
...
def parse_xg(tstats: dict) -> str:
    ...
    v10.5.1: Fixed — now uses STAT_ALIASES which includes 'expected_goals'.
    Also returns actual 0.00 values (not N/A) so xG contributes to GPS.
    """
    # Use the normalized lookup (covers all aliases including 'expected_goals')
...
def send_daily_summary_telegram(client: httpx.Client) -> None:
    ...
    pending = [e for e in all_entries if not e.get("resolved")]
    ...
    lines = [
        ...
        f"{total} resolved, {len(pending)} pending",
        "",
    ...
    # v10.6: Show pending (unresolved) signals
    if pending:
        lines.append("")
        lines.append(f"-- Pending ({len(pending)}) --")
        for e in pending[:10]:
            lines.append(
                f"  {e.get('team_name','?')} ({e.get('league','?')}) "
                f"{e.get('game_minute','?')}' GPS={e.get('gps',0):.0f} "
                f"SOT={e.get('sot','?')}"
            )
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    # v10.5.3: Correct API minute lag using kickoff time
    minute = corrected_minute(fixture, minute)
    ...
    for tid, tname in ((home_tid, home["name"]), (away_tid, away["name"])):
        ...
        # v10.5.3: Track data availability for quality scoring
        # Checks which optional stats the API actually returned (not defaulted to 0)
        _optional_keys = ["expected_goals", "dangerous_attacks", "possession", "big_chances"]
        data_quality = sum(1 for k in _optional_keys if stat_available(tstats, k)) / len(_optional_keys)
        big_chances = safe_int(get_stat(tstats, "big_chances"))
        if not stat_available(tstats, "big_chances"):
            big_chances = 0  # don't count if API doesn't provide it

        # v10.5.3: Log data quality for broken-data league detection
        if data_quality < 0.75:
            _missing = [k for k in _optional_keys if not stat_available(tstats, k)]
            log.debug(f"  DATA QUALITY: {tname} DQ={data_quality:.0%} missing={_missing}")
        ...
        gps, gps_desc, components = calculate_goal_pressure_score(
            ...
            gps_history=history,
            possession=possession,
            big_chances=big_chances,  # v10.5.3
            data_quality=data_quality,  # v10.5.3
        )
        ...
        team_state[(fid, tid)] = {
            ...
            "last_gps": gps,  # v10.5.3: for GPS-based polling acceleration
        }

        if not tier:
            continue

        # === v10.5.1: Scoreline-aware filtering (relaxed) ===
        is_home_sg = (tid == home_tid)
        blocked, block_reason = is_scoreline_blocked(is_home_sg, sh, sa, minute, sot=sot)
        if blocked:
            log.info(
        ...
        # v10.5.1: Allow 2nd team in same fixture to signal.
        # Only skip if THIS team already got a signal.
        ...
        # v10.8: Compute odds value info for 0-0 underdogs
        odds_value_line = ""
        pred = fixture_predictions.get(fid)
        sh_g = fixture["goals"]["home"] or 0
        sa_g = fixture["goals"]["away"] or 0
        if pred and sh_g == 0 and sa_g == 0:
            is_home_team = fixture["teams"]["home"]["id"] == team_id
            team_win_pct = pred["home_pct"] if is_home_team else pred["away_pct"]
            if team_win_pct < 55:
                implied_odds = (100.0 / team_win_pct) if team_win_pct > 0 else 99
                value_tag_odds = "HIGH VALUE" if team_win_pct < 35 else ("GOOD VALUE" if team_win_pct < 45 else "VALUE")
                odds_value_line = f"\n\nPre-match odds: ~{implied_odds:.2f} ({team_win_pct:.0f}% win prob) [{value_tag_odds}]"
        ...
        # v10.7b: CRITICAL signals only (saves 2 credits per EARLY WARNING).
        # Priority: fresh stats > faster polling > player info.
        player_line = ""
        if tier == "CRITICAL":
            try:
        ...
        # v10.5.3: Bulgarian time for signal verification
        bg_now = datetime.now(BULGARIA_TZ).strftime("%H:%M")

        # v10.6: Value window + league tier tags for betting edge assessment
        league_id = fixture["league"]["id"]
        tier_tag = get_league_tier_tag(league_id)
        value_tag = get_value_window_tag(sh, sa)
        if value_tag == "HIGH VALUE":
            value_emoji = "💎"  # gem
        elif value_tag == "GOOD VALUE":
            value_emoji = "✅"  # check
        elif value_tag == "VALUE":
            value_emoji = "👍"  # thumbs up
        else:
            value_emoji = "⚠️"  # warning

        msg = (
            f"{tier_emoji(tier)} {tier} GOAL PRESSURE ({sig_label}) \u23f0 {bg_now}"
            f"  [{value_emoji} {value_tag}] [{tier_tag}]\n\n"
        ...
        outcome_entry = {
            ...
            "team_name": tname,
            "league": league,
            "league_tier": tier_tag,
            "value_window": value_tag,
        ...
        # v10.5.1: In first-signal-only mode, only remove fixture from monitoring
        # when BOTH teams have gotten their signal.
        if is_first_signal_only_mode():
            teams_signaled_now = sum(1 for (f, t) in signaled_teams if f == fid)
            if teams_signaled_now >= 2:
                fast_monitored.discard(fid)
                expire_fast_sot(fid)
                log.info(
                    f"  BOTH-TEAM-DONE: fixture {fid} removed from monitoring "
...
# ============================================================
# POST-GOAL CREDIT RELEASE — 3-TIER SYSTEM (v10.7)
# ============================================================
# Tier logic based on match context and betting value:
#   First half goal (<46'): NO RELEASE — second half odds create fresh value
#   Second half goal (46-69'): SOFT RELEASE — 12-15s polling, still responsive
#   Late goal (70'+): FULL RELEASE — match winding down, credits better elsewhere
# Allows tier upgrade: soft -> full when match crosses 70'

def release_credits_after_goal():
    """v10.7: Tiered credit release when signaled team scores.

    Three tiers based on match context:
    - First half (<46'): NO release. Odds reset in 2nd half, continued
      pressure after scoring is valuable because market underestimates
      teams that keep attacking after taking the lead.
    - Second half 46-69': SOFT release. Clear top acceleration flags,
      drop to 12-15s. Still responsive enough to catch resurgences.
    - 70'+: FULL release. Match winding down, minimal betting value left.
      Clear all flags, drop to base 20-30s.
    """
    if not fast_monitored:
        return

    for fid in list(fast_monitored):
        # Skip if already at the highest release tier for this fixture
        current_tier = goal_released.get(fid)
        ...
        # Check if any signaled team in this fixture has scored
        team_scored = None
        for (f, tid), sig in list(signaled_teams.items()):
            if f != fid:
                continue
            is_home = (tid == home_tid)
            goals_now = sh if is_home else sa
            goals_at_signal = sig.get("goals_at_last_signal", 0)
            if goals_now > goals_at_signal:
                tname = (fixture["teams"]["home"]["name"]
                         if is_home else fixture["teams"]["away"]["name"])
                team_scored = tname
                break

        if not team_scored:
            continue  # no signaled team scored, nothing to release

        # --- Determine release tier based on match context ---
        if minute < 46 or status in ("1H", "HT", "BT"):
            # FIRST HALF: no release — keep fast polling for second half value
            if current_tier is None:
                log.info(
                    f"  GOAL DETECTED (no release): {team_scored} scored at {minute}' "
                    f"(first half) -> keeping fast polling for 2nd half odds"
                )
            continue  # don't set any tier, re-evaluate next cycle

        elif minute < 70:
            # SECOND HALF 46-69': soft release
            if current_tier is None:
                # First time: apply soft release
                pressure_accelerating.discard(fid)
                goal_released[fid] = "soft"
                log.info(
                    f"  SOFT RELEASE: {team_scored} scored at {minute}' "
                    f"-> polling downgraded to 12-15s (still responsive)"
                )
            # else: already soft, nothing to do
            ...
            accelerating_fixtures.discard(fid)
            expire_fast_sot(fid)
            goal_released[fid] = "full"
            if current_tier != "full":
                log.info(
                    f"  FULL RELEASE: fixture {fid} at {minute}' "
                    f"-> polling downgraded to 20-30s (match winding down)"
                )
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v10.8 — Odds-Value Boost for 0-0 Underdogs")
    log.info("=" * 60)
    ...
    log.info("")
    log.info("v10.7 CHANGES:")
    log.info("  0-0 PRIORITY BOOST: 0-0 matches polled faster (higher betting value)")
    log.info("  ODDS VALUE BOOST: 0-0 underdogs ranked higher (prediction API, 1 credit/fixture)")
    log.info("    Win%%<25%%: +150pts | 25-35%%: +100pts | 35-45%%: +60pts | 45-55%%: +25pts | >55%%: 0")
    log.info("    <35': +20 priority | 35-55': +12 | 55-70': +6 | catches pressure before odds move")
    log.info("  PLAYER LOOKUP: CRITICAL signals only (saves ~40 credits/day)")
    log.info("  MOMENTUM GATE: GPS-based CRITICAL now requires accel >= 1 (no stale high-GPS)")
    log.info("  POST-GOAL CREDIT RELEASE (3-tier):")
    log.info("    <46' (first half): NO release — 2nd half odds create fresh value")
    log.info("    46-69' (second half): SOFT release — 12-15s, still responsive")
    log.info("    70'+ (late match): FULL release — 20-30s, credits better elsewhere")
    log.info("  INSTANT SIGNAL MODE: Signals fire the moment API returns SOT/pressure data")
    log.info("    — Sleep cap 30s->10s when monitoring (was 30s even during active matches)")
    log.info("    — Min sleep 8s->3s when monitoring (faster response to interval expiry)")
    log.info("    — Instant re-check: if signal sent or acceleration detected, sleep max 3s")
    log.info("    — This means max ~10s delay from API data change to signal delivery")
    log.info("  POLLING TIERS ALL SLASHED:")
    log.info("    — Pressure/SOT-accelerating: 15s->10s | GPS>=40: 20s->12s | SOT>=3: 20s->12s")
    log.info("    — SOT=2: 30s->15s | Base NORMAL: 30s->20s | Discovery: 600s->300s")
    log.info("")
    log.info("v10.5.3 CHANGES:")
    log.info("  DATA QUALITY: Tracks which optional API stats are available (xG, DA, poss, BC)")
    log.info("    — 0.0-1.0 score in every signal outcome + poll for backtesting")
    log.info("    — Goal: validate whether broken-data leagues have worse hit rates")
    log.info("    — Logged at DEBUG when DQ < 75% (missing stat names shown)")
    log.info("  BIG CHANCES: New 0-5 pt GPS component (human-scored clear opportunities)")
    log.info("    — Added 'big_chances'/'Big Chances' to STAT_ALIASES")
    log.info("    — Scale: 1=1pt, 2=2.5pts, 3=4pts, 4+=5pts")
    log.info("    — Only contributes when API actually provides the stat")
    log.info("    — big_chances count + gps_bc points in outcomes/polls")
    log.info("  STAT_AVAILABLE(): New helper — distinguishes 'API returned 0' from 'field missing'")
    log.info("  MINUTE CORRECTION: Kickoff-time-based fix for API elapsed lag (observed 9+ min)")
    log.info("    — Uses max(api_minute, calculated_from_kickoff) during live play")
    log.info("    — Skips correction during HT/ET (match clock stopped)")
    log.info("    — Prevents inflated per-minute GPS rates from stale API data")
    log.info("  ULTRA-AGGRESSIVE POLLING: Base 90s->20s, high-pressure tiers 60s->10-12s")
    log.info("    — Pressure/SOT-accelerating: 60s->10s | GPS>=40: 60s->12s | SOT>=3: 60s->12s")
    log.info("    — SOT=2: 120s->15s | Discovery: 900s->300s | Sleep cap: 60s->10s")
    log.info("    — Window: 1'-85' | Instant re-check on signal/acceleration: 3s")
    log.info("  SIGNAL TIME: Bulgarian time (HH:MM) shown in every signal header")
    log.info("")
    log.info("v10.5.2 CHANGES:")
    log.info("  SOT VELOCITY: New 0-8 pt component rewarding HOW FAST SOT accumulates")
    log.info("    0.15/m=2pts, 0.25/m=4pts, 0.40/m=6pts, 0.60+/m=8pts")
    log.info("    Uses best of (current-poll rate, 5-min windowed rate)")
    log.info("  xG FIX: 'expected_goals' added to STAT_ALIASES — xG now works!")
    log.info("    (was trying 'Expected Goals' but API returns 'expected_goals')")
    log.info("  DA FIX: DA removed from API for many leagues — points redistributed to xG (15->20 max)")
    log.info("  BOTH-TEAM SIGNALS: +FIRST_ONLY now allows 2nd team in fixture to signal")
    log.info("    (was: fixture dropped after 1st signal. Now: only after BOTH teams signaled)")
    log.info("  SCORELINE RELAXED: Lead threshold 2->3 goals, late cutoff 78'->82', SOT>=5 overrides")
    log.info("  POSSESSION: Added 0-3 pt possession component (moved from end, properly placed)")
    log.info("")
    ...
    log.info("  POLLING: base 30s (was 90s), discovery 600s (NORMAL mode)")
    ...
    log.info("  Components: SOT(28) + InBox(18) + ShotVol(12) + xG(20) + DA(0-8,if) + Corners(4) + BC(0-5,if) + SOTvel(8) + Poss(3) + ACCEL(15)")
    ...
    log.info("  v10.7:   Pressure-accelerating (rate-based 2+ deltas):  10s")
    log.info("  v10.7:   SOT-accelerating (SOT increased last poll):   10s")
    log.info("  v10.7:   GPS >= BUILDING (40), unsignaled:           12s")
    log.info("  v10.7:   SOT >= 3:                                    12s")
    log.info("  v10.7:   SOT >= 1 + fast window:                     15s")
    log.info("  v10.7:   SOT == 2:                                    15s")
    log.info("  v10.7:   SOT == 1:                                    base (20s)")
    log.info("  v10.7:   Instant re-check on signal/accel:            3s")
    ...
    with httpx.Client(timeout=30.0) as client:
        ...
        while True:
            ...
            # v10.7: Track signal count before poll for instant re-check
            _pre_poll_signal_count = len(signals_sent)
            ...
            if fast_monitored and budget != "STOP":
                ...
                # v10.7: Release credits from resolved signals (team scored)
                release_credits_after_goal()
            ...
            # v10.7: Dynamic sleep bounds — instant when monitoring, relaxed when idle
            if fast_monitored:
                sleep_time = max(sleep_time, 3)   # min 3s when actively monitoring
                sleep_time = min(sleep_time, 10)  # max 10s — always ready for next poll
            else:
                sleep_time = max(sleep_time, 8)   # min 8s when idle
                sleep_time = min(sleep_time, 30)  # max 30s when idle

            # v10.7: Instant re-check — if signal or acceleration detected, re-check ASAP
            if fast_monitored and (len(signals_sent) > _pre_poll_signal_count or pressure_accelerating or accelerating_fixtures):
            ...
            # v10.6: Also force-send at end of day (after 00:30 BG) even if pending remain
            now_bg_hour = datetime.now(BULGARIA_TZ).hour
            all_done = (resolved_outcomes and not pending_outcomes
                    and not has_tracked_live and not fast_monitored)
            force_eod = (now_bg_hour >= 0 and now_bg_hour < 2
                    and signal_outcomes and not has_tracked_live and not fast_monitored)
            if all_done or force_eod:
                log_outcome_summary()
