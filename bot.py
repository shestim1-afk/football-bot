...
_REDEPLOY_JSON_FILES: list[str] = [
    "poisson_calibration.json", "field_census.json",
]
...
def redeploy_tracking_check(client: httpx.Client) -> None:
    ...
    try:
        ...
        data_line = (
            ...
            f"{_blocked_n} blocked | {_shadow_n} shadow — files verified"
        )
        if live:
            live_line = (
...
TOP_SOT_RETRY_DELAY = 45          # seconds after the failed line before retry #1
TOP_SOT_RETRY_MAX_ATTEMPTS = 2    # per (fixture, team)
...
def fetch_top_sot_players(
    ...
    try:
        ...
        if not result:
            ...
            # next main-loop tick (~45s) and sends the line once the feed
        ...
        _queue_top_sot_retry(fixture_id, team_id, "error")
        return []


def _queue_top_sot_retry(fid: int, tid: int, reason: str,
                         ev_sot: int | None = None, stats_sot: int | None = None) -> None:
...
def process_top_sot_retries(client: httpx.Client) -> None:
    ...
    try:
        ...
        for (fid, tid) in list(_top_sot_retry_queue.keys()):
            ...
            players = fetch_top_sot_players(client, fid, tid, 3, team_sot_now=_sot_now)
            if players:
                _top_sot_retry_queue.pop((fid, tid), None)
...
def resolve_stale_outcomes(client: httpx.Client) -> int:
    ...
    """
    global signal_outcomes, _resolve_live_fids
...
def process_fixture_stats(client: httpx.Client, fixture: dict) -> None:
    ...
    for tid, tname in ((home_tid, home["name"]), (away_tid, away["name"])):
        ...
        # v10.58: team_sot_now lets the cache rebuild when the team has NEW
        # SOTs since the last build — fresh shooters appear on the line.
        top_sot_players = fetch_top_sot_players(
            client, fid, tid, max_players=3, team_sot_now=sot
        )
        top_sot_str = ""
        if top_sot_players:
            # v10.58: "(n)" = SOT count; "(n shots)" = total shots (fallback
            # when every SOT taker already scored — a scorer is never shown)
            _player_parts = [
                f"{_pname} ({_psot})" if _psot > 0 else f"{_pname} ({_pshots} shots)"
                for _pname, _psot, _pshots in top_sot_players
            ]
            top_sot_str = ", ".join(_player_parts)
            _any_sot = any(_ps > 0 for _, _ps, _ in top_sot_players)
            _line_head = (
                "\U0001f3af Top SOT" if _any_sot
                else "\U0001f3af Top shooters (no SOT yet)"
            )
            if _send_ok:
                send_telegram(
                    client,
                    f"{_line_head} — {tname}: {top_sot_str}\n"
