...
SCHEDULE_RECHECK_INTERVAL = 3 * 3600  # re-fetch every 3h (with or without matches)
...
def get_minutes_until_window() -> int | None:
    """How many real-time seconds until the earliest tracked live
    fixture enters the 20' monitoring window.

    Returns None if no tracked fixture is pre-window (all are >= 20' or
    not live).  Returns 0 if any is already in/near the window.
    v9.6.1: Prevents the bug where a 15-min discovery gap misses the
    20' entry entirely (e.g. Tobol Kostanay at 16' -> next check at 31').
    """
    ...
    for fixture in cached_fixtures:
        ...
        if minute < MINUTE_MIN:
            gap = (MINUTE_MIN - minute) * 60  # seconds until 20'
            if earliest_seconds is None or gap < earliest_seconds:
                earliest_seconds = gap
...
def get_discovery_interval(
    ...
    v9.6.1: When tracked matches are live but pre-20' (no candidates yet),
    use a smart interval based on how soon the earliest match hits 20'.
    This prevents missing the 20' entry window.
    """
    ...
    if not has_candidates:
        # v9.6.1: Smart pre-window discovery
        secs_until = get_minutes_until_window()
        if secs_until is not None and secs_until <= 0:
            # A match is at/near 20' — check NOW (next loop iteration)
            return 60
        if secs_until is not None:
            # Schedule discovery just before the earliest match hits 20'
            # Buffer: check 60s early to account for sleep granularity
            smart_interval = max(60, secs_until - 60)
            # Cap at the mode's normal pre-window interval
            max_pre = {
                "NORMAL": 900,
                "CAREFUL": 1200,
                "STRICT": 1800,
            ...
            return min(smart_interval, max_pre)

        # Fallback: no pre-window matches found (shouldn't happen if
        # has_tracked_live is True and has_candidates is False, but
        # handle gracefully)
        return {
            "NORMAL": 900,
            "CAREFUL": 1200,
...
def get_sot_based_interval(fid: int, base_interval: int) -> int:
    ...
    if not has_state:
        ...
        # v9.5.7: activated at SOT>=1 to catch the 1->2->3 transition
        ...
        # v9.5.8: Stretch SOT=0 interval when credits are tight
        # NORMAL: keep 1.5x (current behavior, no delay risk)
        # CAREFUL/STRICT/EMERGENCY: stretch to 2.5x to save credits
...
def check_monitored_stats(
    ...
    # v9.5.8: SKIP when credits are tight — use cached discovery data.
    # Saves 1 credit per cycle. Signal shows score that's up to 30 min old
    # but SOT detection is unaffected (stats calls still happen).
    skip_batch_refresh = get_budget_mode() in ("CAREFUL", "STRICT", "EMERGENCY")
    if valid_ids and not skip_batch_refresh:
    ...
    elif skip_batch_refresh and valid_ids:
        log.info(
            f"  Skipping batch refresh (credits tight) — using cached data for {len(valid_ids)} fixture(s)"
        )
...
def fetch_daily_active_hours(client: httpx.Client) -> bool:
    ...
    # Already fetched today — re-check every 3h to catch late-added fixtures
    if schedule_date == today_str and (time.time() - schedule_last_fetch) < SCHEDULE_RECHECK_INTERVAL:
        return not schedule_no_matches
    
    if schedule_date == today_str:
        is_recheck = True
        log.info(f"Re-checking schedule for {today_str} (last fetch >3h ago, looking for late additions)...")
    else:
        is_recheck = False
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v9.6.1 — Smart pre-window discovery (no more missed 20' entries)")
    log.info("=" * 60)
    ...
    log.info("")
    log.info("SOT-smart intervals (v9.5.8):")
    log.info("  SOT>=1 + fast window: 120s (unsignaled only — catches 1->2->3)")
    log.info("  SOT>=2 + no window:  180s")
    log.info("  SOT==1:               1.2x base")
    log.info("  SOT==0 NORMAL:        1.5x base (360s — no delay risk)")
    log.info("  SOT==0 CAREFUL+:      2.5x base (saves credits, ~2-4 min delay risk)")
    log.info("  Signaled fixtures: 2x all intervals (both teams signaled)")
    log.info("  Credit-saving: skip batch refresh in CAREFUL/STRICT/EMERGENCY")
