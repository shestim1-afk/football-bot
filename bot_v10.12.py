...
def send_end_of_day_summary(client: httpx.Client) -> None:
    """v10.12: Send end-of-day win rate summary via Telegram.
    ...
    resolved = [e for e in signal_outcomes if e.get("resolved")]
    if not resolved:
        return  # Nothing resolved yet

    # Only send if there are TODAY's resolved outcomes
    # (skip stale outcomes from previous days if any)
    today_signals = []
    for e in resolved:
        sig_date = e.get("signal_clock", "")[:10]
        if sig_date == today_str:
            today_signals.append(e)
    if not today_signals:
        return

    # Also include any still-pending from today
    pending_today = [e for e in signal_outcomes if not e.get("resolved")
                       and e.get("signal_clock", "")[:10] == today_str]
    all_today = today_signals + pending_today

    if not all_today:
        return

    log.info(f"v10.12: Sending end-of-day summary ({len(today_signals)} resolved, {len(pending_today)} pending)")
    stats_text = format_outcome_stats(all_today)
    sent = send_telegram(client, stats_text)
    if sent:
        eod_summary_sent_date = today_str
        log.info(f"v10.12: End-of-day summary sent")
    else:
        log.warning("v10.12: End-of-day Telegram send FAILED")
...
def main():
    log.info("=" * 60)
    log.info("Football Bot v10.12 — fault-tolerant daily summary, league top scorers, season-1 fallback")
    log.info("=" * 60)
